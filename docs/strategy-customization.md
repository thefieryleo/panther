...
Please familiarize yourself with [Panther basics](bot-basics.md) first, which provides overall info on how the bot operates.
...
To get started, use `panther new-strategy --strategy AwesomeStrategy` (you can obviously use your own naming for your strategy).
...
??? Hint "Different template levels"
    `panther new-strategy` has an additional parameter, `--template`, which controls the amount of pre-build information you get in the created strategy. Use `--template minimal` to get an empty strategy without any indicator examples, or `--template advanced` to get a template with most callbacks defined.
...
Panther uses [pandas](https://pandas.pydata.org/) to store/provide the candlestick (OHLCV) data.
...
Out of the box, panther installs the following technical libraries:

* [ta-lib](http://mrjbq7.github.io/ta-lib/)
* [pandas-ta](https://twopirllc.github.io/pandas-ta/)
* [technical](https://github.com/panther/technical/)

Additional technical libraries can be installed as necessary, or custom indicators may be written / invented by the strategy author.

### Strategy startup period

Most indicators have an instable startup period, in which they are either not available (NaN), or the calculation is incorrect. This can lead to inconsistencies, since Panther does not know how long this instable period should be.
...
!!! Warning "Using x calls to get OHLCV"
    ...
    Having this will cause Panther to make multiple calls for the same pair, which will obviously be slower than one network request.
    As a consequence, Panther will take longer to refresh candles - and should therefore be avoided if possible.
    This is capped to 5 total calls to avoid overloading the exchange, or make panther too slow.
...
``` bash
panther backtesting --timerange 20190101-20190201 --timeframe 5m
```
...
``` python
from panther.exchange import timeframe_to_minutes
...
By default, panther will attempt to load strategies from all `.py` files within `user_data/strategies`.

Assuming your strategy is called `AwesomeStrategy`, stored in the file `user_data/strategies/AwesomeStrategy.py`, then you can start panther with `panther trade --strategy AwesomeStrategy`.
Note that we're using the class-name, not the file name.

You can use `panther list-strategies` to see a list of all strategies Panther is able to load (all strategies in the correct folder).
...
??? Example "Fast and easy way to define informative pairs"
    ...
    from panther.strategy import IStrategy, informative
...
This method is used by panther internally to determine the last signal.
...
```python
from panther.strategy import IStrategy, merge_informative_pair
...
!!! Warning "Informative timeframe < timeframe"
    ...
    To use the more detailed information properly, more advanced methods should be applied (which are out of scope for panther documentation, as it'll depend on the respective need).
...
??? Example "Returning a stoploss relative to the open price from the custom stoploss function"
    ...
    from datetime import datetime
    from panther.persistence import Trade
    from panther.strategy import IStrategy, stoploss_from_open
...
??? Example "Returning a stoploss using absolute price from the custom stoploss function"
    ...
    from datetime import datetime
    from panther.persistence import Trade
    from panther.strategy import IStrategy, stoploss_from_absolute
...
Panther locks pairs automatically for the current candle (until that candle is over) when a pair is sold, preventing an immediate re-buy of that pair.
...
Panther has an easy method to do this from within the strategy, by calling `self.lock_pair(pair, until, [reason])`.
...
When buy and sell signals collide (both `'buy'` and `'sell'` are 1), panther will do nothing and ignore the entry (buy) signal. This will avoid trades that buy, and sell immediately. Obviously, this can potentially lead to missed entries.
