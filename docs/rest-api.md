...
The client script only requires the `requests` module, so Panther does not need to be installed on the system.
...
!!! Note
    The below should be done in an application (a Panther REST API client, which fetches info via API), and is not intended to be used on a regular basis.

Panther's REST API also offers JWT (JSON Web Tokens).
You can login using the following command, and subsequently use the resulting access_token.

``` bash
> curl -X POST --user Panther http://localhost:8080/api/v1/token/login
...
This whole section is only necessary in cross-origin cases (where you multiple bot API's running on `localhost:8081`, `localhost:8082`, ...), and want to combine them into one PantherUI instance.
...
??? info "Technical explanation"
    ...
    Since most of the requests to the Panther API must be authenticated, a proper CORS policy is key to avoid security problems.
...
Assuming your application is deployed as `https://pantherui.panther.io/home/` - this would mean that the following configuration becomes necessary:

```jsonc
{
    //...
    "jwt_secret_key": "somethingrandom",
    "CORS_origins": ["https://pantherui.panther.io"],
    //...
}
```

In the following (pretty common) case, PantherUI is accessible on `http://localhost:8080/trade` (this is what you see in your navbar when navigating to pantherUI).
![pantherUI url](assets/pantherui_url.png)
