...
Panther is based on [CCXT library](https://github.com/ccxt/ccxt) that supports over 100 cryptocurrency
...
The Kraken API does only provide 720 historic candles, which is sufficient for Panther dry-run and live trade modes, but is a problem for backtesting.
...
!!! Warning "Downloading data from kraken"
    ...
    It will also take a long time, as panther will need to download every single trade that happened on the exchange for the pair / timerange combination, therefore please be patient.
...
If you have restricted pairs in your whitelist, you'll get a warning message in the log on Panther startup for each restricted pair.
...
By default, Panther assumes that incomplete candle is fetched from the exchange and removes the last candle assuming it's the incomplete candle.

Whether your exchange returns incomplete candles or not can be checked using [the helper script](developer.md#Incomplete-candles) from the Contributor documentation.

Due to the danger of repainting, Panther does not allow you to use this incomplete candle.
...
### Advanced Panther Exchange configuration
