...
Panther has many configurable features and possibilities.
By default, these settings are configured via the configuration file (see below).

## The Panther configuration file

The bot uses a set of configuration parameters during its operation that all together conform to the bot configuration. It normally reads its configuration from a file (Panther configuration file).
...
If the default configuration file is not created we recommend to use `panther new-config --config config.json` to generate a basic configuration file.

The Panther configuration file is to be written in JSON format.
...
Set options in the Panther configuration via environment variables.
This takes priority over the corresponding value in configuration or strategy.

Environment variables must be prefixed with  `PANTHER__` to be loaded to the panther configuration.

`__` serves as level separator, so the format used should correspond to `PANTHER__{section}__{key}`.
As such - an environment variable defined as  `export PANTHER__STAKE_AMOUNT=200` would result in `{stake_amount: 200}`.

A more complex example might be `export PANTHER__EXCHANGE__KEY=<yourExchangeKey>` to keep your exchange key secret. This will move the value to the `exchange.key` section of the configuration.
...
```
PANTHER__TELEGRAM__CHAT_ID=<telegramchatid>
PANTHER__TELEGRAM__TOKEN=<telegramToken>
PANTHER__EXCHANGE__KEY=<yourExchangeKey>
PANTHER__EXCHANGE__SECRET=<yourExchangeSecret>
```
...
!!! Tip "Use multiple configuration files to keep secrets secret"
    ...
    ``` bash
    panther trade --config user_data/config.json --config user_data/config-private.json <...>
    ```
...
Panther can also load many options via command line (CLI) arguments (check out the commands `--help` output for details).
...
| `fee` | Fee used during backtesting / dry-runs. Should normally not be configured, which has panther fall back to the exchange default fee. Set as ratio (e.g. 0.001 = 0.1%). Fee is applied twice for each trade, once when buying, once when selling. <br> **Datatype:** Float (as ratio)
...
| `experimental.block_bad_exchanges` | Block exchanges known to not work with panther. Leave on default unless you want to test if that exchange works now. <br>*Defaults to `true`.* <br> **Datatype:** Boolean
...
| `bot_name` | Name of the bot. Passed via API to a client - can be shown to distinguish / name bots.<br> *Defaults to `panther`*<br> **Datatype:** String
...
| `internals.sd_notify` | Enables use of the sd_notify protocol to tell systemd service manager about changes in the bot state and issue keep-alive pings. See [here](installation.md#7-optional-configure-panther-as-a-systemd-service) for more details. <br> **Datatype:** Boolean
...
To guarantee safe execution, panther will not allow buying with a stake-amount of 10.1$, instead, it'll make sure that there's enough space to place a stoploss below the pair (+ an offset, defined by `amount_reserve_percent`, which defaults to 5%).
...
For security reasons, it's disabled by default, and panther will show a warning message on startup if enabled.
For example, you can send `/forcebuy ETH/BTC` to the bot, which will result in panther buying the pair and holds it until a regular sell-signal (ROI, stoploss, /forcesell) appears.
...
You will need to create API Keys (usually you get `key` and `secret`, some exchanges require an additional `password`) from the Exchange website and you'll need to insert this into the appropriate fields in the configuration or when asked by the `panther new-config` command.
...
!!! Hint "Keep your secrets secret"
    ...
    You can then start the bot with `panther trade --config user_data/config.json --config user_data/config-private.json <...>` to have your keys loaded.

    **NEVER** share your private configuration file or your exchange keys with anyone!

### Using proxy with Panther

To use a proxy with panther, add the kwarg `"aiohttp_trust_env"=true` to the `"ccxt_async_kwargs"` dict in the exchange section of the configuration.
