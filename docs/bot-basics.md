...
This page provides you some basic concepts on how Panther works and operates.
...
All profit calculations of Panther include fees. For Backtesting / Hyperopt / Dry-run modes, the exchange default fee is used (lowest tier on the exchange). For live operations, fees are used as applied by the exchange (this includes BNB rebates etc.).

## Bot execution logic

Starting panther in dry-run or live mode (using `panther trade`) will start the bot and start the bot iteration loop.
