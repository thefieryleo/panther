...
!!! Note
    If you've used `setup.sh`, don't forget to activate your virtual environment (`source .env/bin/activate`) before running panther commands.
...
```
usage: panther trade [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
...
```bash
panther trade -c path/far/far/away/config.json
```
...
```bash
panther trade -c ./config.json
```

and specify both configuration files when running in the normal Live Trade Mode:

```bash
panther trade -c ./config.json -c path/to/secrets/keys.config.json
```
...
Panther allows the creation of a user-data directory using `panther create-userdir --userdir someDirectory`.
...
```bash
panther trade --strategy AwesomeStrategy --strategy-path /some/directory
```
...
```bash
panther trade -c config.json --db-url sqlite:///tradesv3.dry_run.sqlite
```
