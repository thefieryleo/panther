...
The `panther plot-dataframe` subcommand shows an interactive graph with three subplots:
...
```
usage: panther plot-dataframe [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
``` bash
panther plot-dataframe -p BTC/ETH
```

The `-p/--pairs` argument can be used to specify pairs you would like to plot.

!!! Note
    The `panther plot-dataframe` subcommand generates one plot-file per pair.
...
``` bash
panther plot-dataframe --strategy AwesomeStrategy -p BTC/ETH --indicators1 sma ema --indicators2 macd
```
...
``` bash
panther plot-dataframe --strategy AwesomeStrategy -p BTC/ETH XRP/ETH
```

To plot a timerange (to zoom in)

``` bash
panther plot-dataframe --strategy AwesomeStrategy -p BTC/ETH --timerange=20180801-20180805
```

To plot trades stored in a database use `--db-url` in combination with `--trade-source DB`:

``` bash
panther plot-dataframe --strategy AwesomeStrategy --db-url sqlite:///tradesv3.dry_run.sqlite -p BTC/ETH --trade-source DB
```

To plot trades from a backtesting result, use `--export-filename <filename>`

``` bash
panther plot-dataframe --strategy AwesomeStrategy --export-filename user_data/backtest_results/backtest-result.json -p BTC/ETH
```
...
!!! Warning
    `plotly` arguments are only supported with plotly library and will not work with panther-ui.
...
Possible options for the `panther plot-profit` subcommand:

```
usage: panther plot-profit [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                             [-d PATH] [--userdir PATH] [-s NAME]
                             [--strategy-path PATH] [-p PAIRS [PAIRS ...]]
...
``` bash
panther plot-profit  -p LTC/BTC --export-filename user_data/backtest_results/backtest-result.json
```

Use custom database

``` bash
panther plot-profit  -p LTC/BTC --db-url sqlite:///tradesv3.sqlite --trade-source DB
```

``` bash
panther --datadir user_data/data/binance_save/ plot-profit -p LTC/BTC
```
