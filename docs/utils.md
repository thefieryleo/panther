...
Creates the directory structure to hold your files for panther.
...
```
usage: panther create-userdir [-h] [--userdir PATH] [--reset]
...
```
usage: panther new-config [-h] [-c PATH]
...
!!! Warning
    Only vital questions are asked. Panther offers a lot more configuration possibilities, which are listed in the [Configuration documentation](configuration.md#configuration-parameters)

### Create config examples

```
$ panther new-config --config config_binance.json
...
``` output
usage: panther new-strategy [-h] [--userdir PATH] [-s NAME]
...
```bash
panther new-strategy --strategy AwesomeStrategy
```

With custom user directory

```bash
panther new-strategy --userdir ~/.panther/ --strategy AwesomeStrategy
```

Using the advanced template (populates all optional functions and methods)

```bash
panther new-strategy --strategy AwesomeStrategy --template advanced
```
...
```
usage: panther list-strategies [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                 [-d PATH] [--userdir PATH]
...
``` bash
panther list-strategies
```

Example: Search strategies  directory within the userdir.

``` bash
panther list-strategies --userdir ~/.panther/
```

Example: Search dedicated strategy path.

``` bash
panther list-strategies --strategy-path ~/.panther/strategies/
```
...
```
usage: panther list-exchanges [-h] [-1] [-a]
...
```
$ panther list-exchanges
Exchanges available for Panther:
...
* Example: see all exchanges supported by the ccxt library (including 'bad' ones, i.e. those that are known to not work with Panther):
```
$ panther list-exchanges -a
...
```
usage: panther list-timeframes [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH] [--userdir PATH] [--exchange EXCHANGE] [-1]
...
```
$ panther list-timeframes -c config_binance.json
...
Timeframes available for the exchange `binance`: 1m, 3m, 5m, 15m, 30m, 1h, 2h, 4h, 6h, 8h, 12h, 1d, 3d, 1w, 1M
```

* Example: enumerate exchanges available for Panther and print timeframes supported by each of them:
```
$ for i in `panther list-exchanges -1`; do panther list-timeframes --exchange $i; done
```
...
For pairs traded by Panther the pair quote currency is defined by the value of the `stake_currency` configuration setting.
...
```
usage: panther list-markets [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
usage: panther list-pairs [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                            [-d PATH] [--userdir PATH] [--exchange EXCHANGE]
                            [--print-list] [--print-json] [-1] [--print-csv]
                            [--base BASE_CURRENCY [BASE_CURRENCY ...]]
...
```
$ panther list-pairs --quote USD --print-json
```
...
```
$ panther list-pairs -c config_binance.json --all --base BTC ETH --quote USDT USD --print-list
```

* Print all markets on exchange "Kraken", in the tabular format:

```
$ panther list-markets --exchange kraken --all
```
...
```
usage: panther test-pairlist [-h] [-c PATH]
                               [--quote QUOTE_CURRENCY [QUOTE_CURRENCY ...]]
                               [-1] [--print-json]
...
```
panther test-pairlist --config config.json --quote USDT BTC
```
...
Run panther in webserver mode.
Panther will start the webserver and allow PantherUI to start and control backtesting processes.
...
PantherUI will also show the backtesting results.

```
usage: panther webserver [-h] [-v] [--logfile FILE] [-V] [-c PATH] [-d PATH]
                           [--userdir PATH]
...
```
usage: panther backtesting-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                                  [-d PATH] [--userdir PATH]
...
```
usage: panther hyperopt-list [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
```
panther hyperopt-list --profitable --no-details
```
...
```
usage: panther hyperopt-show [-h] [-v] [--logfile FILE] [-V] [-c PATH]
                               [-d PATH] [--userdir PATH] [--best]
...
```
usage: panther show-trades [-h] [-v] [--logfile FILE] [-V] [-c PATH]
...
``` bash
panther show-trades --db-url sqlite:///tradesv3.sqlite --trade-ids 2 3 --print-json
```
