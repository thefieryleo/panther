...
from panther.constants import CANCEL_REASON, MATH_CLOSE_PREC, UNLIMITED_STAKE_AMOUNT
from panther.enums import RPCMessageType, RunMode, SellType, State
from panther.exceptions import (DependencyException, ExchangeError, InsufficientFundsError,
                                  InvalidOrderException, OperationalException, PricingError,
                                  TemporaryError)
from panther.pantherbot import PantherBot
from panther.persistence import Order, PairLocks, Trade
from panther.persistence.models import PairLock
from panther.strategy.interface import SellCheckTuple
from panther.worker import Worker
from tests.conftest import (create_mock_trades, get_patched_pantherbot, get_patched_worker,
...
def patch_RPCManager(mocker) -> MagicMock:
    ...
    rpc_mock = mocker.patch('panther.pantherbot.RPCManager.send_msg', MagicMock())
    return rpc_mock


# Unit tests

def test_pantherbot_state(mocker, default_conf_usdt, markets) -> None:
    mocker.patch('panther.exchange.Exchange.markets', PropertyMock(return_value=markets))
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    assert panther.state is State.RUNNING

    default_conf_usdt.pop('initial_state')
    panther = PantherBot(default_conf_usdt)
    assert panther.state is State.STOPPED


def test_process_stopped(mocker, default_conf_usdt) -> None:

    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    coo_mock = mocker.patch('panther.pantherbot.PantherBot.cancel_all_open_orders')
    panther.process_stopped()
...
def test_bot_cleanup(mocker, default_conf_usdt, caplog) -> None:
    mock_cleanup = mocker.patch('panther.pantherbot.cleanup_db')
    coo_mock = mocker.patch('panther.pantherbot.PantherBot.cancel_all_open_orders')
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    panther.cleanup()
    ...
    panther.config['cancel_open_orders_on_exit'] = True
    panther.cleanup()
    assert coo_mock.call_count == 1
...
def test_order_dict(default_conf_usdt, mocker, runmode, caplog) -> None:
    ...
    panther = PantherBot(conf)
    if runmode == RunMode.LIVE:
        assert not log_has_re(".*stoploss_on_exchange .* dry-run", caplog)
    assert panther.strategy.order_types['stoploss_on_exchange']
    ...
    panther = PantherBot(conf)
    assert not panther.strategy.order_types['stoploss_on_exchange']
    assert not log_has_re(".*stoploss_on_exchange .* dry-run", caplog)
...
def test_get_trade_stake_amount(default_conf_usdt, mocker) -> None:
    ...
    result = panther.wallets.get_trade_stake_amount('ETH/USDT')
...
def test_check_available_stake_amount(
    ...
    for i in range(0, max_open):
        ...
        if expected[i] is not None:
            ...
            panther.execute_entry('ETH/USDT', result)
        else:
            with pytest.raises(DependencyException):
                panther.wallets.get_trade_stake_amount('ETH/USDT')
...
def test_edge_called_in_process(mocker, edge_conf) -> None:
    ...
    patch_exchange(mocker)
    panther = PantherBot(edge_conf)
    patch_get_signal(panther)
    panther.process()
    assert panther.active_pair_whitelist == ['NEO/BTC', 'LTC/BTC']
...
def test_edge_overrides_stake_amount(mocker, edge_conf) -> None:
    ...
    assert panther.wallets.get_trade_stake_amount(
        'NEO/BTC', panther.edge) == (999.9 * 0.5 * 0.01) / 0.20
    assert panther.wallets.get_trade_stake_amount(
        'LTC/BTC', panther.edge) == (999.9 * 0.5 * 0.01) / 0.21
...
def test_edge_overrides_stoploss(limit_buy_order_usdt, fee, caplog, mocker,
    ...
    panther = PantherBot(edge_conf)
    panther.active_pair_whitelist = ['NEO/BTC']
    patch_get_signal(panther)
    panther.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    assert panther.handle_trade(trade) is not ignore_strat_sl
...
def test_create_trade_minimal_amount(
    ...
    panther = PantherBot(default_conf_usdt)
    panther.config['stake_amount'] = stake_amount
    patch_get_signal(panther)

    if create:
        assert panther.create_trade('ETH/USDT')
        if amount_enough:
        ...
        assert not panther.create_trade('ETH/USDT')
        if not max_open_trades:
            assert panther.wallets.get_trade_stake_amount('ETH/USDT', panther.edge) == 0
...
def test_handle_protections(mocker, default_conf_usdt, fee):
    ...
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    panther.protections._protection_handlers[1].global_stop = MagicMock(
        return_value=(True, arrow.utcnow().shift(hours=1).datetime, "asdf"))
    create_mock_trades(fee)
    panther.handle_protections('ETC/BTC')
    send_msg_mock = panther.rpc.send_msg
...
def test_create_trade_no_signal(default_conf_usdt, fee, mocker) -> None:
    ...
    panther = PantherBot(default_conf_usdt)
    patch_get_signal(panther, value=(False, False, None, None))
...
def test_create_trades_preopen(default_conf_usdt, ticker_usdt, fee, mocker,
    ...
    panther.execute_entry('ETH/USDT', default_conf_usdt['stake_amount'])
    panther.execute_entry('NEO/BTC', default_conf_usdt['stake_amount'])
    ...
    assert panther.create_trade('ETH/USDT')
    assert panther.create_trade('NEO/BTC')
...
def test_process_exchange_failures(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    patch_get_signal(worker.panther)
...
def test_process_operational_exception(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    assert worker.panther.state == State.RUNNING

    worker._process_running()
    assert worker.panther.state == State.STOPPED
...
def test_process_trade_no_whitelist_pair(default_conf_usdt, ticker_usdt, limit_buy_order_usdt,
    ...
    assert pair not in panther.active_pair_whitelist
    panther.process()
    assert pair in panther.active_pair_whitelist
    # Make sure each pair is only in the list once
    assert len(panther.active_pair_whitelist) == len(set(panther.active_pair_whitelist))
...
def test_process_informative_pairs_added(default_conf_usdt, ticker_usdt, mocker) -> None:
    ...
    mocker.patch(
        'panther.strategy.interface.IStrategy.get_signal',
        return_value=(False, False, '', '')
    )
    mocker.patch('time.sleep', return_value=None)

    panther = PantherBot(default_conf_usdt)
    panther.strategy.informative_pairs = inf_pairs
    # patch_get_signal(panther)
...
def test_execute_entry(mocker, default_conf_usdt, fee, limit_buy_order_usdt,
    ...
    patch_exchange(mocker)
    panther = PantherBot(default_conf_usdt)
    panther.strategy.confirm_trade_entry = MagicMock(return_value=False)
    stake_amount = 2
    bid = 0.11
    ...
    fix_price = 0.06
    assert panther.execute_entry(pair, stake_amount, fix_price)
    ...
    mocker.patch('panther.exchange.Exchange.create_order',
    ...
    panther.strategy.custom_stake_amount = lambda **kwargs: 150.0
    assert panther.execute_entry(pair, stake_amount)
    ...
    panther.strategy.custom_stake_amount = lambda **kwargs: 20 / 0
    assert panther.execute_entry(pair, stake_amount)
    ...
    # Fail to get price...
    mocker.patch('panther.exchange.Exchange.get_rate', MagicMock(return_value=0.0))

    with pytest.raises(PricingError, match="Could not determine buy price."):
        panther.execute_entry(pair, stake_amount)

    # In case of custom entry price
    mocker.patch('panther.exchange.Exchange.get_rate', return_value=0.50)
    limit_buy_order_usdt['status'] = 'open'
    limit_buy_order_usdt['id'] = '5566'
    panther.strategy.custom_entry_price = lambda **kwargs: 0.508
    assert panther.execute_entry(pair, stake_amount)
    ...
    panther.strategy.custom_entry_price = lambda **kwargs: None
    ...
    panther.strategy.custom_entry_price = lambda **kwargs: "string price"
    assert panther.execute_entry(pair, stake_amount)
...
def test_execute_entry_confirm_error(mocker, default_conf_usdt, fee, limit_buy_order_usdt) -> None:
    ...
    panther.strategy.confirm_trade_entry = MagicMock(side_effect=ValueError)
    assert panther.execute_entry(pair, stake_amount)

    limit_buy_order_usdt['id'] = '222'
    panther.strategy.confirm_trade_entry = MagicMock(side_effect=Exception)
    assert panther.execute_entry(pair, stake_amount)
...
def test_add_stoploss_on_exchange(mocker, default_conf_usdt, limit_buy_order_usdt) -> None:
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch('panther.pantherbot.PantherBot.handle_trade', MagicMock(return_value=True))
    mocker.patch('panther.exchange.Exchange.fetch_order', return_value=limit_buy_order_usdt)
    mocker.patch('panther.exchange.Exchange.get_trades_for_order', return_value=[])
    mocker.patch('panther.pantherbot.PantherBot.get_real_amount',
...
def test_handle_stoploss_on_exchange(mocker, default_conf_usdt, fee, caplog,
    ...
    assert panther.handle_stoploss_on_exchange(trade) is False
    assert stoploss.call_count == 1
    assert trade.stoploss_order_id == "13434334"
    ...
    mocker.patch('panther.exchange.Binance.fetch_stoploss_order', hanging_stoploss_order)
    ...
    mocker.patch('panther.exchange.Binance.fetch_stoploss_order', canceled_stoploss_order)
    stoploss.reset_mock()
    ...
    mocker.patch('panther.exchange.Binance.fetch_stoploss_order', stoploss_order_hit)
    assert panther.handle_stoploss_on_exchange(trade) is True
    ...
    mocker.patch(
        'panther.exchange.Binance.stoploss',
        side_effect=ExchangeError()
    )
    trade.is_open = True
    panther.handle_stoploss_on_exchange(trade)
    ...
    trade.stoploss_order_id = 100
    stoploss.reset_mock()
    mocker.patch('panther.exchange.Binance.fetch_stoploss_order',
    ...
    trade.is_open = False
    stoploss.reset_mock()
    mocker.patch('panther.exchange.Exchange.fetch_order')
    mocker.patch('panther.exchange.Binance.stoploss', stoploss)
    assert panther.handle_stoploss_on_exchange(trade) is False
...
def test_create_stoploss_order_invalid_order(mocker, default_conf_usdt, caplog, fee,
    ...
    trade = Trade.query.first()
    caplog.clear()
    panther.create_stoploss_order(trade, 200)
    assert trade.stoploss_order_id is None
...
def test_create_stoploss_order_insufficient_funds(mocker, default_conf_usdt, caplog, fee,
    ...
    mock_insuf = mocker.patch('panther.pantherbot.PantherBot.handle_insufficient_funds')
    mocker.patch.multiple(
        'panther.exchange.Exchange',
        fetch_ticker=MagicMock(return_value={
            'bid': 1.9,
...
def test_handle_stoploss_on_exchange_trailing(mocker, default_conf_usdt, fee,
    ...
    # setting stoploss
    panther.strategy.stoploss = -0.05

    # setting stoploss_on_exchange_interval to 60 seconds
    panther.strategy.order_types['stoploss_on_exchange_interval'] = 60
    ...
    mocker.patch('panther.exchange.Binance.fetch_stoploss_order', stoploss_order_hanging)

    # stoploss initially at 5%
    assert panther.handle_trade(trade) is False
    assert panther.handle_stoploss_on_exchange(trade) is False

    # price jumped 2x
    mocker.patch(
        'panther.exchange.Exchange.fetch_ticker',
    ...
    mocker.patch('panther.exchange.Binance.cancel_stoploss_order', cancel_order_mock)
    mocker.patch('panther.exchange.Binance.stoploss', stoploss_order_mock)
    ...
    panther.strategy.order_types['stoploss_on_exchange_interval'] = 0
    ...
    stoploss_order_mock.assert_called_once_with(
        amount=27.39726027,
        pair='ETH/USDT',
        order_types=panther.strategy.order_types,
        stop_price=4.4 * 0.95
    )
    ...
    assert panther.handle_trade(trade) is True
...
def test_handle_stoploss_on_exchange_trailing_error(
    ...
    mocker.patch('panther.exchange.Binance.cancel_stoploss_order',
                 side_effect=InvalidOrderException())
    mocker.patch('panther.exchange.Binance.fetch_stoploss_order',
                 return_value=stoploss_order_hanging)
    panther.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
    ...
    caplog.clear()
    cancel_mock = mocker.patch("panther.exchange.Binance.cancel_stoploss_order", MagicMock())
    mocker.patch("panther.exchange.Binance.stoploss", side_effect=ExchangeError())
    panther.handle_trailing_stoploss_on_exchange(trade, stoploss_order_hanging)
...
def test_handle_stoploss_on_exchange_custom_stop(
    ...
    # setting stoploss
    panther.strategy.custom_stoploss = lambda *args, **kwargs: -0.04
...
def test_tsl_on_exchange_compatible_with_edge(mocker, edge_conf, fee,
    ...
    # setting stoploss
    panther.strategy.stoploss = -0.02
    ...
    panther.active_pair_whitelist = panther.edge.adjust(panther.active_pair_whitelist)
    ...
    mocker.patch('panther.exchange.Exchange.fetch_stoploss_order', stoploss_order_hanging)
    ...
    mocker.patch('panther.exchange.Exchange.cancel_stoploss_order', cancel_order_mock)
    mocker.patch('panther.exchange.Binance.stoploss', stoploss_order_mock)

    # price goes down 5%
    mocker.patch('panther.exchange.Exchange.fetch_ticker', MagicMock(return_value={
        'bid': 2.19 * 0.95,
        'ask': 2.2 * 0.95,
        'last': 2.19 * 0.95
    }))
...
def test_enter_positions(mocker, default_conf_usdt, return_value, side_effect,
    ...
    mock_ct = mocker.patch(
        'panther.pantherbot.PantherBot.create_trade',
        MagicMock(
...
def test_exit_positions_exception(mocker, default_conf_usdt, limit_buy_order_usdt, caplog) -> None:
    ...
    mocker.patch(
        'panther.pantherbot.PantherBot.handle_trade',
...
def test_update_trade_state(mocker, default_conf_usdt, limit_buy_order_usdt, caplog) -> None:
    ...
    mocker.patch('panther.pantherbot.PantherBot.get_real_amount', return_value=90.81)
    ...
    mocker.patch('panther.pantherbot.PantherBot.get_real_amount', side_effect=ValueError)
    mocker.patch('panther.exchange.Exchange.fetch_order', return_value=limit_buy_order_usdt_new)
    res = panther.update_trade_state(trade, '123')
    # Cancelled empty
    assert res is True
...
def test_update_trade_state_withorderdict(default_conf_usdt, trades_for_order, limit_buy_order_usdt,
    ...
    mocker.patch('panther.exchange.Exchange.get_trades_for_order', return_value=trades_for_order)
    # fetch_order should not be called!!
    mocker.patch('panther.exchange.Exchange.fetch_order', MagicMock(side_effect=ValueError))
    patch_exchange(mocker)
    ...
    panther.update_trade_state(trade, '123456', limit_buy_order_usdt)
...
def test_update_trade_state_exception(mocker, default_conf_usdt,
    ...
    mocker.patch(
        'panther.pantherbot.PantherBot.get_real_amount',
        side_effect=DependencyException()
    )
    panther.update_trade_state(trade, trade.open_order_id)
...
def test_update_trade_state_orderexception(mocker, default_conf_usdt, caplog) -> None:
    ...
    # Test raise of OperationalException exception
    grm_mock = mocker.patch("panther.pantherbot.PantherBot.get_real_amount", MagicMock())
    panther.update_trade_state(trade, trade.open_order_id)
...
def test_update_trade_state_sell(default_conf_usdt, trades_for_order, limit_sell_order_usdt_open,
    ...
    mocker.patch('panther.wallets.Wallets.update', wallet_mock)
    ...
    panther.update_trade_state(trade, trade.open_order_id, limit_sell_order_usdt)
...
def test_handle_trade(default_conf_usdt, limit_buy_order_usdt, limit_sell_order_usdt_open,
    ...
    trade.update(limit_buy_order_usdt)
    assert trade.is_open is True
    panther.wallets.update()

    patch_get_signal(panther, value=(False, True, None, 'sell_signal1'))
    assert panther.handle_trade(trade) is True
...
def test_handle_overlapping_signals(default_conf_usdt, ticker_usdt, limit_buy_order_usdt_open,
    ...
    panther = PantherBot(default_conf_usdt)
    patch_get_signal(panther, value=(True, True, None, None))
    panther.strategy.min_roi_reached = MagicMock(return_value=False)
    ...
    patch_get_signal(panther, value=(False, False, None, None))
    assert panther.handle_trade(trades[0]) is False
    trades = Trade.query.all()
    nb_trades = len(trades)
    assert nb_trades == 1
    assert trades[0].is_open is True
    ...
    trades = Trade.query.all()
    assert panther.handle_trade(trades[0]) is True
...
def test_handle_trade_roi(default_conf_usdt, ticker_usdt, limit_buy_order_usdt_open,
    ...
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    patch_get_signal(panther)
    panther.strategy.min_roi_reached = MagicMock(return_value=True)
    ...
    patch_get_signal(panther, value=(False, False, None, None))
    assert panther.handle_trade(trade)
...
def test_handle_trade_use_sell_signal(default_conf_usdt, ticker_usdt, limit_buy_order_usdt_open,
    ...
    patch_get_signal(panther, value=(False, False, None, None))
    assert not panther.handle_trade(trade)
...
def test_close_trade(default_conf_usdt, ticker_usdt, limit_buy_order_usdt,
    ...
    with pytest.raises(DependencyException, match=r'.*closed trade.*'):
        panther.handle_trade(trade)


def test_bot_loop_start_called_once(mocker, default_conf_usdt, caplog):
    ftbot = get_patched_pantherbot(mocker, default_conf_usdt)
    mocker.patch('panther.pantherbot.PantherBot.create_trade')
...
def test_check_handle_timedout_buy_usercustom(default_conf_usdt, ticker_usdt, limit_buy_order_old,
    ...
    panther.check_handle_timedout()
    assert cancel_order_mock.call_count == 0

    # Return false - trade remains open
    panther.strategy.check_buy_timeout = MagicMock(return_value=False)
    panther.check_handle_timedout()
    assert cancel_order_mock.call_count == 0
    trades = Trade.query.filter(Trade.open_order_id.is_(open_trade.open_order_id)).all()
    nb_trades = len(trades)
    assert nb_trades == 1
    assert panther.strategy.check_buy_timeout.call_count == 1

    # Raise Keyerror ... (no impact on trade)
    panther.strategy.check_buy_timeout = MagicMock(side_effect=KeyError)
    panther.check_handle_timedout()
    assert cancel_order_mock.call_count == 0
    trades = Trade.query.filter(Trade.open_order_id.is_(open_trade.open_order_id)).all()
    nb_trades = len(trades)
    ...
    panther.strategy.check_buy_timeout = MagicMock(return_value=True)
...
def test_check_handle_timedout_buy(default_conf_usdt, ticker_usdt, limit_buy_order_old, open_trade,
    ...
    assert panther.strategy.check_buy_timeout.call_count == 0
...
def test_check_handle_timedout_sell_usercustom(default_conf_usdt, ticker_usdt, limit_sell_order_old,
    ...
    cancel_order_mock = MagicMock()
    patch_exchange(mocker)
    et_mock = mocker.patch('panther.pantherbot.PantherBot.execute_trade_exit')
    mocker.patch.multiple(
        'panther.exchange.Exchange',
        fetch_ticker=ticker_usdt,
    ...
    panther.strategy.check_sell_timeout = MagicMock(return_value=False)
    # Return false - No impact
    panther.check_handle_timedout()
    assert cancel_order_mock.call_count == 0
    assert rpc_mock.call_count == 0
    assert open_trade.is_open is False
    assert panther.strategy.check_sell_timeout.call_count == 1

    panther.strategy.check_sell_timeout = MagicMock(side_effect=KeyError)
    ...
    # Return True - sells!
    panther.strategy.check_sell_timeout = MagicMock(return_value=True)
    panther.check_handle_timedout()
    assert cancel_order_mock.call_count == 1
    assert rpc_mock.call_count == 1
    ...
    mocker.patch('panther.persistence.Trade.get_exit_order_count', return_value=1)
    mocker.patch('panther.pantherbot.PantherBot.execute_trade_exit',
                 side_effect=DependencyException)
    panther.check_handle_timedout()
...
def test_check_handle_timedout_sell(default_conf_usdt, ticker_usdt, limit_sell_order_old, mocker,
    ...
    assert panther.strategy.check_sell_timeout.call_count == 0
...
def test_check_handle_timedout_partial_fee(default_conf_usdt, ticker_usdt, open_trade, caplog, fee,
    ...
    mocker.patch('panther.wallets.Wallets.get_free', MagicMock(return_value=0))
    patch_exchange(mocker)
    mocker.patch.multiple(
        'panther.exchange.Exchange',
        fetch_ticker=ticker_usdt,
...
def test_handle_cancel_enter(mocker, caplog, default_conf_usdt, limit_buy_order_usdt) -> None:
    ...
    mocker.patch('panther.exchange.Exchange.cancel_order_with_result', cancel_order_mock)

    panther = PantherBot(default_conf_usdt)
    panther._notify_enter_cancel = MagicMock()
    ...
    assert panther.handle_cancel_enter(trade, limit_buy_order_usdt, reason)
    assert cancel_order_mock.call_count == 1

    cancel_order_mock.reset_mock()
    caplog.clear()
    limit_buy_order_usdt['filled'] = 0.01
    assert not panther.handle_cancel_enter(trade, limit_buy_order_usdt, reason)
    assert cancel_order_mock.call_count == 0
...
def test_handle_cancel_enter_exchanges(mocker, caplog, default_conf_usdt,
    ...
    cancel_order_mock = mocker.patch(
        'panther.exchange.Exchange.cancel_order_with_result',
        return_value=limit_buy_order_canceled_empty)
    nofiy_mock = mocker.patch('panther.pantherbot.PantherBot._notify_enter_cancel')
    panther = PantherBot(default_conf_usdt)

    reason = CANCEL_REASON['TIMEOUT']
    trade = MagicMock()
    trade.pair = 'LTC/ETH'
    assert panther.handle_cancel_enter(trade, limit_buy_order_canceled_empty, reason)
    assert cancel_order_mock.call_count == 0
...
def test_handle_cancel_exit_limit(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch('panther.exchange.Exchange.get_rate', return_value=0.245441)
    ...
    order = {'remaining': 1,
             ...
             'status': "open"}
    reason = CANCEL_REASON['TIMEOUT']
    assert panther.handle_cancel_exit(trade, order, reason)
    assert cancel_order_mock.call_count == 1
...
def test_handle_cancel_exit_cancel_exception(mocker, default_conf_usdt) -> None:
    ...
    patch_exchange(mocker)
    mocker.patch(
        'panther.exchange.Exchange.cancel_order_with_result', side_effect=InvalidOrderException())
    ...
    order = {'remaining': 1,
             'amount': 1,
             'status': "open"}
    assert panther.handle_cancel_exit(trade, order, reason) == 'error cancelling order'
...
def test_execute_trade_exit_up(default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_up, mocker
    ...
    panther = PantherBot(default_conf_usdt)
    patch_get_signal(panther)
    panther.strategy.confirm_trade_exit = MagicMock(return_value=False)
    ...
    # Prevented sell ...
    panther.execute_trade_exit(trade=trade, limit=ticker_usdt_sell_up()['bid'],
                                 sell_reason=SellCheckTuple(sell_type=SellType.ROI))
    assert rpc_mock.call_count == 0
    assert panther.strategy.confirm_trade_exit.call_count == 1
    assert id(panther.strategy.confirm_trade_exit.call_args_list[0][1]['trade']) != id(trade)
    assert panther.strategy.confirm_trade_exit.call_args_list[0][1]['trade'].id == trade.id
...
def test_execute_trade_exit_down(default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_down,
    ...
    panther.execute_trade_exit(trade=trade, limit=ticker_usdt_sell_down()['bid'],
...
def test_execute_trade_exit_custom_exit_price(default_conf_usdt, ticker_usdt, fee,
    ...
    panther.strategy.custom_exit_price = lambda **kwargs: 2.25
...
def test_execute_trade_exit_sloe_cancel_exception(
    ...
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    mocker.patch('panther.exchange.Exchange.cancel_stoploss_order',
                 side_effect=InvalidOrderException())
    mocker.patch('panther.wallets.Wallets.get_free', MagicMock(return_value=300))
    create_order_mock = MagicMock(side_effect=[
        {'id': '12345554'},
        {'id': '12345555'},
    ])
    ...
    panther.config['dry_run'] = False
    trade.stoploss_order_id = "abcd"

    panther.execute_trade_exit(trade=trade, limit=1234,
                                 sell_reason=SellCheckTuple(sell_type=SellType.STOP_LOSS))
    assert create_order_mock.call_count == 2
...
def test_may_execute_trade_exit_after_stoploss_on_exchange_hit(default_conf_usdt, ticker_usdt, fee,
    ...
    mocker.patch('panther.exchange.Exchange.fetch_stoploss_order', stoploss_executed)
...
def test_execute_trade_exit_market_order(default_conf_usdt, ticker_usdt, fee,
    ...
    panther.config['order_types']['sell'] = 'market'
...
def test_execute_trade_exit_insufficient_funds_error(default_conf_usdt, ticker_usdt, fee,
    ...
    sell_reason = SellCheckTuple(sell_type=SellType.ROI)
    assert not panther.execute_trade_exit(trade=trade, limit=ticker_usdt_sell_up()['bid'],
...
def test_sell_profit_only(
    ...
    if sell_type == SellType.SELL_SIGNAL.value:
        ...
        panther.strategy.stop_loss_reached = MagicMock(return_value=SellCheckTuple(
    ...
    panther.wallets.update()
    patch_get_signal(panther, value=(False, True, None, None))
    assert panther.handle_trade(trade) is handle_first

    if handle_second:
        panther.strategy.sell_profit_offset = 0.0
        assert panther.handle_trade(trade) is True
...
def test_sell_not_enough_balance(default_conf_usdt, limit_buy_order_usdt, limit_buy_order_usdt_open,
    ...
    trade.update(limit_buy_order_usdt)
    patch_get_signal(panther, value=(False, True, None, None))
    mocker.patch('panther.wallets.Wallets.get_free', MagicMock(return_value=trade.amount * 0.985))
...
def test__safe_exit_amount(default_conf_usdt, fee, caplog, mocker, amount_wallet, has_err):
    ...
    mocker.patch('panther.wallets.Wallets.get_free', MagicMock(return_value=amount_wallet))
    wallet_update = mocker.patch('panther.wallets.Wallets.update')
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    if has_err:
        with pytest.raises(DependencyException, match=r"Not enough amount to sell."):
            assert panther._safe_exit_amount(trade.pair, trade.amount)
    else:
        wallet_update.reset_mock()
        assert panther._safe_exit_amount(trade.pair, trade.amount) == amount_wallet
        assert log_has_re(r'.*Falling back to wallet-amount.', caplog)
        assert wallet_update.call_count == 1
        caplog.clear()
        wallet_update.reset_mock()
        assert panther._safe_exit_amount(trade.pair, amount_wallet) == amount_wallet
...
def test_locked_pairs(default_conf_usdt, ticker_usdt, fee, ticker_usdt_sell_down, mocker,
    ...
    assert panther.strategy.is_pair_locked(trade.pair)
...
def test_trailing_stop_loss(default_conf_usdt, limit_buy_order_usdt_open,
    ...
    mocker.patch('panther.exchange.Exchange.fetch_ticker',
                 MagicMock(return_value={
                     'bid': 2.0 * 1.5,
                     'ask': 2.0 * 1.5,
                     'last': 2.0 * 1.5
...
def test_disable_ignore_roi_if_buy_signal(default_conf_usdt, limit_buy_order_usdt,
    ...
    patch_get_signal(panther, value=(True, False, None, None))
    assert panther.handle_trade(trade) is True
...
def test_get_real_amount_quote(default_conf_usdt, trades_for_order, buy_order_fee, fee, caplog,
    ...
    assert panther.get_real_amount(trade, buy_order_fee) == amount - (amount * 0.001)
...
def test_get_real_amount_quote_dust(default_conf_usdt, trades_for_order, buy_order_fee, fee,
                                    caplog, mocker):
    mocker.patch('panther.exchange.Exchange.get_trades_for_order', return_value=trades_for_order)
    walletmock = mocker.patch('panther.wallets.Wallets.update')
    mocker.patch('panther.wallets.Wallets.get_free', return_value=8.1122)
    amount = sum(x['amount'] for x in trades_for_order)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
    ...
    assert panther.get_real_amount(trade, buy_order_fee) == amount
...
def test_get_real_amount(
    ...
    if not use_ticker_usdt_rate:
        mocker.patch('panther.exchange.Exchange.fetch_ticker', side_effect=ExchangeError)

    caplog.clear()
    assert panther.get_real_amount(trade, buy_order) == amount - fee_reduction_amount
...
def test_get_real_amount_multi(
    ...
    assert panther.get_real_amount(trade, buy_order_fee) == expected_amount
    assert log_has(
        (
            'Applying fee on amount for Trade(id=None, pair=LTC/ETH, amount=8.00000000, '
...
def test_get_real_amount_invalid_order(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    assert panther.get_real_amount(trade, limit_buy_order_usdt) == amount


def test_get_real_amount_fees_order(default_conf_usdt, market_buy_order_usdt_doublefee,
                                    fee, mocker):

    tfo_mock = mocker.patch('panther.exchange.Exchange.get_trades_for_order', return_value=[])
    mocker.patch('panther.exchange.Exchange.get_valid_pair_combination', return_value='BNB/USDT')
    mocker.patch('panther.exchange.Exchange.fetch_ticker', return_value={'last': 200})
    trade = Trade(
        pair='LTC/USDT',
        amount=30.0,
        exchange='binance',
    ...
    assert panther.get_real_amount(trade, market_buy_order_usdt_doublefee) == 30.0
...
def test_get_real_amount_wrong_amount(default_conf_usdt, trades_for_order, buy_order_fee, fee,
    ...
    with pytest.raises(DependencyException, match=r"Half bought\? Amounts don't match"):
        panther.get_real_amount(trade, limit_buy_order_usdt)
...
def test_get_real_amount_open_trade(default_conf_usdt, fee, mocker):
    ...
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    assert panther.get_real_amount(trade, order) == amount
...
def test_apply_fee_conditional(default_conf_usdt, fee, mocker,
    ...
    walletmock = mocker.patch('panther.wallets.Wallets.update')
    mocker.patch('panther.wallets.Wallets.get_free', return_value=wallet)
    trade = Trade(
        pair='LTC/ETH',
        amount=amount,
        exchange='binance',
    ...
    walletmock.reset_mock()
    # Amount is kept as is
    assert panther.apply_fee_conditional(trade, 'LTC', amount, fee_abs) == amount_exp
    assert walletmock.call_count == 1
...
def test_order_book_depth_of_market(
    ...
    patch_RPCManager(mocker)
    patch_exchange(mocker)
    mocker.patch('panther.exchange.Exchange.fetch_l2_order_book', order_book_l2)
    mocker.patch.multiple(
        'panther.exchange.Exchange',
        fetch_ticker=ticker_usdt,
        create_order=MagicMock(return_value=limit_buy_order_usdt_open),
...
def test_order_book_bid_strategy1(mocker, default_conf_usdt, order_book_l2, exception_thrown,
    ...
    if exception_thrown:
        with pytest.raises(PricingError):
            panther.exchange.get_rate('ETH/USDT', refresh=True, side="buy")
        assert log_has_re(
            r'Buy Price at location 1 from orderbook could not be determined.', caplog)
    else:
        assert panther.exchange.get_rate('ETH/USDT', refresh=True, side="buy") == 0.043935
...
def test_check_depth_of_market_buy(default_conf_usdt, mocker, order_book_l2) -> None:
    ...
    assert panther._check_depth_of_market_buy('ETH/USDT', conf) is False
...
def test_order_book_ask_strategy(
    ...
    mocker.patch('panther.exchange.Exchange.fetch_l2_order_book',
...
def test_startup_state(default_conf_usdt, mocker):
    ...
    mocker.patch('panther.exchange.Exchange.exchange_has', MagicMock(return_value=True))
    worker = get_patched_worker(mocker, default_conf_usdt)
    assert worker.panther.state is State.RUNNING
...
def test_startup_trade_reinit(default_conf_usdt, edge_conf, mocker):
    ...
    mocker.patch('panther.persistence.Trade.stoploss_reinitialization', reinit_mock)
    ...
    ftbot = get_patched_pantherbot(mocker, edge_conf)
    ftbot.startup()
...
def test_sync_wallet_dry_run(mocker, default_conf_usdt, ticker_usdt, fee, limit_buy_order_usdt_open,
    ...
    bot = get_patched_pantherbot(mocker, default_conf_usdt)
...
def test_cancel_all_open_orders(mocker, default_conf_usdt, fee, limit_buy_order_usdt,
    ...
    buy_mock = mocker.patch('panther.pantherbot.PantherBot.handle_cancel_enter')
    sell_mock = mocker.patch('panther.pantherbot.PantherBot.handle_cancel_exit')

    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    create_mock_trades(fee)
    trades = Trade.query.all()
    assert len(trades) == MOCK_TRADE_COUNT
    panther.cancel_all_open_orders()
...
def test_check_for_open_trades(mocker, default_conf_usdt, fee):
    ...
    panther.check_for_open_trades()
    assert panther.rpc.send_msg.call_count == 0
    ...
    panther.check_for_open_trades()
    assert panther.rpc.send_msg.call_count == 1
    assert 'Handle these trades manually' in panther.rpc.send_msg.call_args[0][0]['status']
...
def test_startup_update_open_orders(mocker, default_conf_usdt, fee, caplog):
    ...
    panther.startup_update_open_orders()
    ...
    mocker.patch('panther.exchange.Exchange.fetch_order', return_value=matching_buy_order)
    panther.startup_update_open_orders()
...
def test_update_closed_trades_without_assigned_fees(mocker, default_conf_usdt, fee):
    ...
    mocker.patch('panther.exchange.Exchange.fetch_order_or_stoploss_order',
                 side_effect=[
    ...
    panther.update_closed_trades_without_assigned_fees()
...
def test_reupdate_enter_order_fees(mocker, default_conf_usdt, fee, caplog):
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    mock_uts = mocker.patch('panther.pantherbot.PantherBot.update_trade_state')

    create_mock_trades(fee)
    trades = Trade.get_trades().all()

    panther.reupdate_enter_order_fees(trades[0])
    ...
    panther.reupdate_enter_order_fees(trade)
    assert log_has_re(r"Trying to reupdate buy fees for .*", caplog)
...
def test_handle_insufficient_funds(mocker, default_conf_usdt, fee):
    panther = get_patched_pantherbot(mocker, default_conf_usdt)
    mock_rlo = mocker.patch('panther.pantherbot.PantherBot.refind_lost_order')
    mock_bof = mocker.patch('panther.pantherbot.PantherBot.reupdate_enter_order_fees')
    create_mock_trades(fee)
    trades = Trade.get_trades().all()

    # Trade 0 has only a open buy order, no closed order
    panther.handle_insufficient_funds(trades[0])
    ...
    panther.handle_insufficient_funds(trades[1])
    ...
    panther.handle_insufficient_funds(trades[2])
    assert mock_rlo.call_count == 1
    assert mock_bof.call_count == 0
    ...
    panther.handle_insufficient_funds(trades[3])
    assert mock_rlo.call_count == 0
    assert mock_bof.call_count == 1
...
def test_refind_lost_order(mocker, default_conf_usdt, fee, caplog):
    ...
    mock_fo = mocker.patch('panther.exchange.Exchange.fetch_order_or_stoploss_order',
    ...
    panther.refind_lost_order(trade)
    order = mock_order_1()
    ...
    panther.refind_lost_order(trades[4])
...
def test_get_valid_price(mocker, default_conf_usdt) -> None:
    ...
    patch_exchange(mocker)
    panther = PantherBot(default_conf_usdt)
    panther.config['custom_price_max_distance_ratio'] = 0.02
    ...
    valid_price_from_string = panther.get_valid_price(custom_price_string, proposed_price)
    valid_price_from_badstring = panther.get_valid_price(custom_price_badstring, proposed_price)
    valid_price_from_int = panther.get_valid_price(custom_price_int, proposed_price)
    valid_price_from_float = panther.get_valid_price(custom_price_float, proposed_price)

    valid_price_at_max_alwd = panther.get_valid_price(custom_price_over_max_alwd, proposed_price)
    valid_price_at_min_alwd = panther.get_valid_price(custom_price_under_min_alwd, proposed_price)
...
def test_position_adjust(mocker, default_conf_usdt, fee) -> None:
    ...
    mocker.patch('panther.exchange.Exchange.fetch_order_or_stoploss_order',
                 MagicMock(return_value=open_dca_order_1))
    assert panther.execute_entry(pair, stake_amount, trade=trade)
    ...
    mocker.patch('panther.exchange.Exchange.create_order', fetch_order_mm)
    mocker.patch('panther.exchange.Exchange.fetch_order', fetch_order_mm)
    mocker.patch('panther.exchange.Exchange.fetch_order_or_stoploss_order', fetch_order_mm)
    panther.update_closed_trades_without_assigned_fees()
...
def test_process_open_trade_positions_exception(mocker, default_conf_usdt, fee, caplog) -> None:
    ...
    mocker.patch('panther.pantherbot.PantherBot.check_and_call_adjust_trade_position',
                 side_effect=DependencyException())

    create_mock_trades(fee)

    panther.process_open_trade_positions()
