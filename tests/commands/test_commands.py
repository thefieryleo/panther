...
from panther.commands import (start_backtesting_show, start_convert_data, start_convert_trades,
...
from panther.commands.deploy_commands import (clean_ui_subdir, download_and_install_ui,
...
def test_start_trading_fail(mocker, caplog):
    ...
    mocker.patch("panther.worker.Worker.run", MagicMock(side_effect=OperationalException))

    mocker.patch("panther.worker.Worker.__init__", MagicMock(return_value=None))

    exitmock = mocker.patch("panther.worker.Worker.exit", MagicMock())
    args = [
        'trade',
    ...
    exitmock.reset_mock()
    caplog.clear()
    mocker.patch("panther.worker.Worker.__init__", MagicMock(side_effect=OperationalException))
    start_trading(get_args(args))
...
def test_start_webserver(mocker, caplog):
    ...
    api_server_mock = mocker.patch("panther.rpc.api_server.ApiServer", )
...
def test_list_exchanges(capsys):
    ...
    assert re.match(r"Exchanges available for Panther.*", captured.out)
...
def test_list_markets(mocker, markets_static, capsys):
    ...
    mocker.patch('panther.exchange.Exchange.markets', PropertyMock(side_effect=ValueError))
    # Test --one-column
    args = [
        "list-markets",
        '--config', 'config_examples/config_bittrex.example.json',
...
def test_create_datadir(caplog, mocker):
    ...
    cud = mocker.patch("panther.commands.deploy_commands.create_userdata_dir", MagicMock())
    csf = mocker.patch("panther.commands.deploy_commands.copy_sample_files", MagicMock())
    args = [
        "create-userdir",
        "--userdir",
        "/temp/panther/test"
...
def test_start_new_strategy(mocker, caplog):
    ...
    mocker.patch('panther.commands.deploy_commands.setup_utils_configuration')
...
def test_start_install_ui(mocker):
    clean_mock = mocker.patch('panther.commands.deploy_commands.clean_ui_subdir')
    get_url_mock = mocker.patch('panther.commands.deploy_commands.get_ui_download_url',
                                return_value=('https://example.com/whatever', '0.0.1'))
    download_mock = mocker.patch('panther.commands.deploy_commands.download_and_install_ui')
    mocker.patch('panther.commands.deploy_commands.read_ui_version', return_value=None)
    args = [
        "install-ui",
    ]
...
def test_clean_ui_subdir(mocker, tmpdir, caplog):
    mocker.patch("panther.commands.deploy_commands.Path.is_dir",
                 side_effect=[True, True])
    mocker.patch("panther.commands.deploy_commands.Path.is_file",
                 side_effect=[False, True])
    rd_mock = mocker.patch("panther.commands.deploy_commands.Path.rmdir")
    ul_mock = mocker.patch("panther.commands.deploy_commands.Path.unlink")

    mocker.patch("panther.commands.deploy_commands.Path.glob",
...
def test_download_and_install_ui(mocker, tmpdir):
    ...
    mocker.patch("panther.commands.deploy_commands.requests.get", return_value=requests_mock)

    mocker.patch("panther.commands.deploy_commands.Path.is_dir",
                 side_effect=[True, False])
    wb_mock = mocker.patch("panther.commands.deploy_commands.Path.write_bytes")
...
def test_get_ui_download_url(mocker):
    ...
    get_mock = mocker.patch("panther.commands.deploy_commands.requests.get",
...
def test_download_data_keyboardInterrupt(mocker, caplog, markets):
    dl_mock = mocker.patch('panther.commands.data_commands.refresh_backtest_ohlcv_data',
                           MagicMock(side_effect=KeyboardInterrupt))
    patch_exchange(mocker)
    mocker.patch(
        'panther.exchange.Exchange.markets', PropertyMock(return_value=markets)
    )
    args = [
        "download-data",
        "--exchange", "binance",
...
def test_download_data_no_markets(mocker, caplog):
    ...
    mocker.patch(
        'panther.exchange.Exchange.markets', PropertyMock(return_value={})
    )
    args = [
        "download-data",
        "--exchange", "binance",
...
def test_download_data_no_exchange(mocker, caplog):
    mocker.patch('panther.commands.data_commands.refresh_backtest_ohlcv_data',
                 MagicMock(return_value=["ETH/BTC", "XRP/BTC"]))
    patch_exchange(mocker)
    mocker.patch(
        'panther.exchange.Exchange.markets', PropertyMock(return_value={})
...
def test_download_data_trades(mocker, caplog):
    dl_mock = mocker.patch('panther.commands.data_commands.refresh_backtest_trades_data',
                           MagicMock(return_value=[]))
    convert_mock = mocker.patch('panther.commands.data_commands.convert_trades_to_ohlcv',
                                MagicMock(return_value=[]))
    patch_exchange(mocker)
    mocker.patch(
        'panther.exchange.Exchange.markets', PropertyMock(return_value={})
...
def test_hyperopt_list(mocker, capsys, caplog, saved_hyperopt_results, tmpdir):
    ...
    mocker.patch(
        'panther.optimize.hyperopt_tools.HyperoptTools._test_hyperopt_results_exist',
        return_value=True
        )
    ...
    mocker.patch(
        'panther.optimize.hyperopt_tools.HyperoptTools._read_results',
...
def test_hyperopt_show(mocker, capsys, saved_hyperopt_results):
    ...
    mocker.patch('panther.commands.hyperopt_commands.show_backtest_result')
...
def test_convert_data(mocker, testdatadir):
    ohlcv_mock = mocker.patch("panther.commands.data_commands.convert_ohlcv_format")
    trades_mock = mocker.patch("panther.commands.data_commands.convert_trades_format")
    args = [
        "convert-data",
        "--format-from",
        "json",
...
def test_show_trades(mocker, fee, capsys, caplog):
    mocker.patch("panther.persistence.init_db")
...
def test_backtesting_show(mocker, testdatadir, capsys):
    sbr = mocker.patch('panther.optimize.optimize_reports.show_backtest_results')
    args = [
        "backtesting-show",
        "--export-filename",
