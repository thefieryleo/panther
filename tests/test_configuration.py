...
from panther.configuration import Configuration, check_exchange, validate_config_consistency
from panther.configuration.config_validation import validate_config_schema
from panther.configuration.deprecated_settings import (check_conflicting_settings,
                                                         process_deprecated_setting,
                                                         process_removed_setting,
                                                         process_temporary_deprecated_settings)
from panther.configuration.environment_vars import flat_vars_to_nested_dict
from panther.configuration.load_config import load_config_file, load_file, log_config_error_range
from panther.constants import DEFAULT_DB_DRYRUN_URL, DEFAULT_DB_PROD_URL, ENV_VAR_PREFIX
from panther.enums import RunMode
from panther.exceptions import OperationalException
from panther.loggers import FTBufferingHandler, _set_loggers, setup_logging, setup_logging_pre
...
def test_load_config_file(default_conf, mocker, caplog) -> None:
    ...
    file_mock = mocker.patch('panther.configuration.load_config.open', mocker.mock_open(
...
def test_load_config_file_error(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('panther.configuration.load_config.open', mocker.mock_open(read_data=filedata))
...
def test_load_config_combine_dicts(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch(
        'panther.configuration.configuration.load_config_file',
        configsmock
    )
...
def test_from_config(default_conf, mocker, caplog) -> None:
    ...
    config_files = [conf1, conf2]
    mocker.patch('panther.configuration.configuration.create_datadir', lambda c, x: x)

    configsmock = MagicMock(side_effect=config_files)
    mocker.patch('panther.configuration.configuration.load_config_file', configsmock)
...
def test_load_config_file_exception(mocker) -> None:
    mocker.patch(
        'panther.configuration.configuration.open',
...
def test_setup_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch(
        'panther.configuration.configuration.create_userdata_dir',
    ...
    assert log_has('Using user-data directory: {} ...'.format(Path("/tmp/panther")), caplog)
    assert 'user_data_dir' in config
...
def test_check_exchange(default_conf, caplog) -> None:
    # Test an officially supported by Panther team exchange
    default_conf['runmode'] = RunMode.DRY_RUN
    default_conf.get('exchange').update({'name': 'BITTREX'})
    assert check_exchange(default_conf)
    assert log_has_re(r"Exchange .* is officially supported by the Panther development team\.",
                      caplog)
    caplog.clear()
    ...
    assert log_has_re(r"Exchange .* is known to the the ccxt library, available for the bot, "
                      ...
                      r"by the Panther development team\. .*", caplog)
    caplog.clear()

    # Test a 'bad' exchange, which known to have serious problems
    default_conf.get('exchange').update({'name': 'bitmex'})
    with pytest.raises(OperationalException,
                       match=r"Exchange .* will not work with Panther\..*"):
...
def test_cli_verbose_with_params(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('panther.loggers._set_loggers', MagicMock)
    arglist = ['trade', '-vvv']
    args = Arguments(arglist).get_parsed_arg()
...
def test_pairlist_resolving_fallback(mocker):
    ...
    mocker.patch("panther.configuration.configuration.load_file",
...
def test_flat_vars_to_nested_dict(caplog):
    ...
    test_args = {
        'PANTHER__EXCHANGE__SOME_SETTING': 'true',
        'PANTHER__EXCHANGE__SOME_FALSE_SETTING': 'false',
        'PANTHER__EXCHANGE__CONFIG__whatever': 'sometime',
        'PANTHER__ASK_STRATEGY__PRICE_SIDE': 'bid',
        'PANTHER__ASK_STRATEGY__cccc': '500',
        'PANTHER__STAKE_AMOUNT': '200.05',
        'PANTHER__TELEGRAM__CHAT_ID': '2151',
    ...
    assert log_has("Loading variable 'PANTHER__EXCHANGE__SOME_SETTING'", caplog)
