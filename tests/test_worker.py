...
def test_worker_state(mocker, default_conf, markets) -> None:
    ...
    assert worker.panther.state is State.STOPPED


def test_worker_running(mocker, default_conf, caplog) -> None:
    mock_throttle = MagicMock()
    mocker.patch('panther.worker.Worker._throttle', mock_throttle)
    mocker.patch('panther.persistence.Trade.stoploss_reinitialization', MagicMock())
    ...
    assert worker.panther.strategy
    assert worker.panther.strategy.dp
    assert isinstance(worker.panther.strategy.dp, DataProvider)
...
def test_worker_stopped(mocker, default_conf, caplog) -> None:
    ...
    worker = get_patched_worker(mocker, default_conf)
    worker.panther.state = State.STOPPED
...
def test_worker_heartbeat_running(default_conf, mocker, caplog):
    ...
    worker.panther.state = State.RUNNING
