...
from panther.constants import LAST_BT_RESULT_FN
from panther.data.btanalysis import (BT_DATA_COLUMNS, analyze_trade_parallelism, calculate_csum,
                                       ...
                                       load_trades_from_db)
from panther.data.history import load_data, load_pair_history
...
def test_get_latest_backtest_filename(testdatadir, mocker):
    ...
    mocker.patch("panther.data.btanalysis.json_load", return_value={})
...
def test_load_backtest_metadata(mocker, testdatadir):
    ...
    mocker.patch('panther.data.btanalysis.get_backtest_metadata_filename')
    mocker.patch('panther.data.btanalysis.json_load', side_effect=Exception())
...
def test_load_backtest_data_old_format(testdatadir, mocker):
    ...
    mocker.patch('panther.data.btanalysis.load_backtest_stats', return_value=[])
...
def test_load_trades_from_db(default_conf, fee, mocker):
    ...
    init_mock = mocker.patch('panther.data.btanalysis.init_db', MagicMock())
...
def test_load_trades(default_conf, mocker):
    db_mock = mocker.patch("panther.data.btanalysis.load_trades_from_db", MagicMock())
    bt_mock = mocker.patch("panther.data.btanalysis.load_backtest_data", MagicMock())
