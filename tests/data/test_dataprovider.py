...
from panther.exceptions import ExchangeError, OperationalException
...
def test_historic_ohlcv(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch("panther.data.dataprovider.load_pair_history", historymock)
...
def test_historic_ohlcv_dataformat(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch("panther.data.history.hdf5datahandler.HDF5DataHandler._ohlcv_load", hdf5loadmock)
    mocker.patch("panther.data.history.jsondatahandler.JsonDataHandler._ohlcv_load", jsonloadmock)
...
def test_refresh(mocker, default_conf, ohlcv_history):
    ...
    mocker.patch("panther.exchange.Exchange.refresh_latest_ohlcv", refresh_mock)
...
def test_ticker(mocker, default_conf, tickers):
    ...
    mocker.patch("panther.exchange.Exchange.fetch_ticker", ticker_mock)
    exchange = get_patched_exchange(mocker, default_conf)
    dp = DataProvider(default_conf, exchange)
