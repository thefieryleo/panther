...
from panther.constants import AVAILABLE_DATAHANDLERS
from panther.data.converter import ohlcv_to_dataframe
from panther.data.history.hdf5datahandler import HDF5DataHandler
from panther.data.history.history_utils import (_download_pair_history, _download_trades_history,
                                                  _load_cached_data_for_updating,
                                                  convert_trades_to_ohlcv, get_timerange, load_data,
                                                  load_pair_history, refresh_backtest_ohlcv_data,
                                                  refresh_backtest_trades_data, refresh_data,
                                                  validate_backtest_data)
from panther.data.history.idatahandler import IDataHandler, get_datahandler, get_datahandlerclass
from panther.data.history.jsondatahandler import JsonDataHandler, JsonGzDataHandler
from panther.exchange import timeframe_to_minutes
from panther.misc import file_dump_json
...
def test_load_data_7min_timeframe(mocker, caplog, default_conf, testdatadir) -> None:
    ...
    assert log_has(
        ...
        'Use `panther download-data` to download the data', caplog
    )


def test_load_data_1min_timeframe(ohlcv_history, mocker, caplog, testdatadir) -> None:
    mocker.patch('panther.exchange.Exchange.get_historic_ohlcv', return_value=ohlcv_history)
...
def test_load_data_startup_candles(mocker, caplog, default_conf, testdatadir) -> None:
    ltfmock = mocker.patch(
        'panther.data.history.jsondatahandler.JsonDataHandler._ohlcv_load',
...
def test_load_data_with_new_pair_1min(ohlcv_history_list, mocker, caplog,
    ...
    """
    tmpdir1 = Path(tmpdir)
    mocker.patch('panther.exchange.Exchange.get_historic_ohlcv', return_value=ohlcv_history_list)
...
@pytest.mark.parametrize("pair,expected_result", [
    ("ETH/BTC", 'panther/hello/world/ETH_BTC-5m.json'),
    ("Fabric Token/ETH", 'panther/hello/world/Fabric_Token_ETH-5m.json'),
    ("ETHH20", 'panther/hello/world/ETHH20-5m.json'),
    (".XBTBON2H", 'panther/hello/world/_XBTBON2H-5m.json'),
    ("ETHUSD.d", 'panther/hello/world/ETHUSD_d-5m.json'),
    ("ACC_OLD/BTC", 'panther/hello/world/ACC_OLD_BTC-5m.json'),
])
def test_json_pair_data_filename(pair, expected_result):
    fn = JsonDataHandler._pair_data_filename(Path('panther/hello/world'), pair, '5m')
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)
    fn = JsonGzDataHandler._pair_data_filename(Path('panther/hello/world'), pair, '5m')
    assert isinstance(fn, Path)
    assert fn == Path(expected_result + '.gz')


@pytest.mark.parametrize("pair,expected_result", [
    ("ETH/BTC", 'panther/hello/world/ETH_BTC-trades.json'),
    ("Fabric Token/ETH", 'panther/hello/world/Fabric_Token_ETH-trades.json'),
    ("ETHH20", 'panther/hello/world/ETHH20-trades.json'),
    (".XBTBON2H", 'panther/hello/world/_XBTBON2H-trades.json'),
    ("ETHUSD.d", 'panther/hello/world/ETHUSD_d-trades.json'),
    ("ACC_OLD_BTC", 'panther/hello/world/ACC_OLD_BTC-trades.json'),
])
def test_json_pair_trades_filename(pair, expected_result):
    fn = JsonDataHandler._pair_trades_filename(Path('panther/hello/world'), pair)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result)

    fn = JsonGzDataHandler._pair_trades_filename(Path('panther/hello/world'), pair)
    assert isinstance(fn, Path)
    assert fn == Path(expected_result + '.gz')
...
def test_download_pair_history2(mocker, default_conf, testdatadir) -> None:
    ...
    json_dump_mock = mocker.patch(
        'panther.data.history.jsondatahandler.JsonDataHandler.ohlcv_store',
        return_value=None)
    mocker.patch('panther.exchange.Exchange.get_historic_ohlcv', return_value=tick)
    exchange = get_patched_exchange(mocker, default_conf)
...
def test_download_backtesting_data_exception(mocker, caplog, default_conf, tmpdir) -> None:
    mocker.patch('panther.exchange.Exchange.get_historic_ohlcv',
...
def test_refresh_backtest_ohlcv_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch('panther.data.history.history_utils._download_pair_history',
                           MagicMock())
    mocker.patch(
...
def test_refresh_backtest_trades_data(mocker, default_conf, markets, caplog, testdatadir):
    dl_mock = mocker.patch('panther.data.history.history_utils._download_trades_history',
                           MagicMock())
    mocker.patch(
        'panther.exchange.Exchange.markets', PropertyMock(return_value=markets)
    )
...
def test_download_trades_history(trades_history, mocker, default_conf, testdatadir, caplog,
    ...
    mocker.patch('panther.exchange.Exchange.get_historic_trades',
                 ght_mock)
    exchange = get_patched_exchange(mocker, default_conf)
