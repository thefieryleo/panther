...
from panther.edge import Edge, PairInfo
from panther.enums import SellType
from panther.exceptions import OperationalException
from tests.conftest import get_patched_pantherbot, log_has
...
def test_edge_results(edge_conf, mocker, caplog, data) -> None:
    ...
    """
    panther = get_patched_pantherbot(mocker, edge_conf)
    edge = Edge(edge_conf, panther.exchange, panther.strategy)
...
def test_edge_process_downloaded_data(mocker, edge_conf):
    panther = get_patched_pantherbot(mocker, edge_conf)
    mocker.patch('panther.exchange.Exchange.get_fee', MagicMock(return_value=0.001))
    mocker.patch('panther.edge.edge_positioning.refresh_data', MagicMock())
    mocker.patch('panther.edge.edge_positioning.load_data', mocked_load_data)
    edge = Edge(edge_conf, panther.exchange, panther.strategy)
...
def test_edge_process_no_data(mocker, edge_conf, caplog):
    ...
    mocker.patch('panther.exchange.Exchange.get_fee', MagicMock(return_value=0.001))
    mocker.patch('panther.edge.edge_positioning.refresh_data', MagicMock())
    mocker.patch('panther.edge.edge_positioning.load_data', MagicMock(return_value={}))
    edge = Edge(edge_conf, panther.exchange, panther.strategy)
...
def test_edge_process_no_trades(mocker, edge_conf, caplog):
    panther = get_patched_pantherbot(mocker, edge_conf)
    mocker.patch('panther.exchange.Exchange.get_fee', return_value=0.001)
    mocker.patch('panther.edge.edge_positioning.refresh_data', )
    mocker.patch('panther.edge.edge_positioning.load_data', mocked_load_data)
    # Return empty
    mocker.patch('panther.edge.Edge._find_trades_for_stoploss_range', return_value=[])
    edge = Edge(edge_conf, panther.exchange, panther.strategy)
...
def test_edge_process_no_pairs(mocker, edge_conf, caplog):
    ...
    mocker.patch('panther.pantherbot.validate_config_consistency')

    panther = get_patched_pantherbot(mocker, edge_conf)
    fee_mock = mocker.patch('panther.exchange.Exchange.get_fee', return_value=0.001)
    mocker.patch('panther.edge.edge_positioning.refresh_data')
    mocker.patch('panther.edge.edge_positioning.load_data', mocked_load_data)
    # Return empty
    mocker.patch('panther.edge.Edge._find_trades_for_stoploss_range', return_value=[])
...
def test_edge_init_error(mocker, edge_conf,):
    ...
    with pytest.raises(OperationalException,  match='Edge works only with unlimited stake amount'):
        get_patched_pantherbot(mocker, edge_conf)
...
def test_process_expectancy(mocker, edge_conf, fee, risk_reward_ratio, expectancy):
    ...
    panther.exchange.get_fee = get_fee
    edge = Edge(edge_conf, panther.exchange, panther.strategy)
