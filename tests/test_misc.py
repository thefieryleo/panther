...
from panther.misc import (decimals_per_coin, file_dump_json, file_load_json, format_ms_time,
...
def test_file_dump_json(mocker) -> None:
    file_open = mocker.patch('panther.misc.open', MagicMock())
    json_dump = mocker.patch('rapidjson.dump', MagicMock())
    file_dump_json(Path('somefile'), [1, 2, 3])
    assert file_open.call_count == 1
    assert json_dump.call_count == 1
    file_open = mocker.patch('panther.misc.gzip.open', MagicMock())
    json_dump = mocker.patch('rapidjson.dump', MagicMock())
...
@pytest.mark.parametrize('conn_url,expected', [
    ...
    ("sqlite:////panther/user_data/tradesv3.sqlite",
     "sqlite:////panther/user_data/tradesv3.sqlite"),
])
