...
from panther.persistence import LocalTrade, Order, Trade, clean_dry_run_db, init_db
...
def test_init_custom_db_url(default_conf, tmpdir):
    ...
    filename = f"{tmpdir}/panther2_test.sqlite"
...
def test_init_prod_db(default_conf, mocker):
    ...
    create_engine_mock = mocker.patch('panther.persistence.models.create_engine', MagicMock())
...
def test_init_dryrun_db(default_conf, tmpdir):
    filename = f"{tmpdir}/panther2_prod.sqlite"
    assert not Path(filename).is_file()
    default_conf.update({
        'dry_run': True,
...
def test_migrate_new(mocker, default_conf, fee, caplog):
    ...
    mocker.patch('panther.persistence.models.create_engine', lambda *args, **kwargs: engine)
