...
from panther.commands import start_plot_dataframe, start_plot_profit
from panther.configuration import TimeRange
from panther.data import history
from panther.data.btanalysis import create_cum_profit, load_backtest_data
from panther.exceptions import OperationalException
from panther.plot.plotting import (add_areas, add_indicators, add_profit, create_plotconfig,
...
def test_generate_candlestick_graph_no_signals_no_trades(default_conf, mocker, testdatadir, caplog):
    row_mock = mocker.patch('panther.plot.plotting.add_indicators',
                            MagicMock(side_effect=fig_generating_mock))
    trades_mock = mocker.patch('panther.plot.plotting.plot_trades',
                               MagicMock(side_effect=fig_generating_mock))
...
def test_generate_Plot_filename():
    ...
    assert fn == "panther-plot-UNITTEST_BTC-5m.html"


def test_generate_plot_file(mocker, caplog):
    fig = generate_empty_figure()
    plot_mock = mocker.patch("panther.plot.plotting.plot", MagicMock())
    store_plot_file(fig, filename="panther-plot-UNITTEST_BTC-5m.html",
                    directory=Path("user_data/plot"))

    expected_fn = str(Path("user_data/plot/panther-plot-UNITTEST_BTC-5m.html"))
...
def test_generate_profit_graph(testdatadir):
    ...
    assert fig.layout.title.text == "Panther Profit plot"
...
def test_start_plot_dataframe(mocker):
    aup = mocker.patch("panther.plot.plotting.load_and_plot_trades", MagicMock())
    args = [
        "plot-dataframe",
...
def test_start_plot_profit(mocker):
    aup = mocker.patch("panther.plot.plotting.plot_profit", MagicMock())
    args = [
        "plot-profit",
        "--config", "config_examples/config_bittrex.example.json",
        "--pairs", "ETH/BTC"
