...
from panther.constants import AVAILABLE_PAIRLISTS
...
from panther.plugins.pairlistmanager import PairListManager
from panther.resolvers import PairListResolver
from tests.conftest import (create_mock_trades, get_patched_exchange, get_patched_pantherbot,
...
def test_log_cached(mocker, static_pl_conf, markets, tickers):
    mocker.patch.multiple('panther.exchange.Exchange',
                          markets=PropertyMock(return_value=markets),
                          exchange_has=MagicMock(return_value=True),
                          get_tickers=tickers
                          )
    panther = get_patched_pantherbot(mocker, static_pl_conf)
    logmock = MagicMock()
    # Assign starting whitelist
    pl = panther.pairlists._pairlist_handlers[0]
...
def test_load_pairlist_noexist(mocker, markets, default_conf):
    ...
    plm = PairListManager(panther.exchange, default_conf)
    with pytest.raises(OperationalException,
                       match=r"Impossible to load Pairlist 'NonexistingPairList'. "
                             r"This class does not exist or contains Python code errors."):
        PairListResolver.load_pairlist('NonexistingPairList', panther.exchange, plm,
                                       default_conf, {}, 1)


def test_load_pairlist_verify_multi(mocker, markets_static, default_conf):
    panther = get_patched_pantherbot(mocker, default_conf)
    mocker.patch('panther.exchange.Exchange.markets', PropertyMock(return_value=markets_static))
    plm = PairListManager(panther.exchange, default_conf)
...
def test_refresh_market_pair_not_in_whitelist(mocker, markets, static_pl_conf):
    ...
    mocker.patch('panther.exchange.Exchange.markets', PropertyMock(return_value=markets))
    panther.pairlists.refresh_pairlist()
    # List ordered by BaseVolume
    whitelist = ['ETH/BTC', 'TKN/BTC']
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(panther.pairlists.whitelist)
    # Ensure config dict hasn't been changed
    assert (static_pl_conf['exchange']['pair_whitelist'] ==
            panther.config['exchange']['pair_whitelist'])
...
def test_refresh_static_pairlist(mocker, markets, static_pl_conf):
    ...
    # Ensure all except those in whitelist are removed
    assert set(whitelist) == set(panther.pairlists.whitelist)
    assert static_pl_conf['exchange']['pair_blacklist'] == panther.pairlists.blacklist
...
def test_refresh_static_pairlist_noexist(mocker, markets, static_pl_conf, pairs, expected, caplog):
    ...
    # Ensure all except those in whitelist are removed
    assert set(expected) == set(panther.pairlists.whitelist)
    assert static_pl_conf['exchange']['pair_blacklist'] == panther.pairlists.blacklist
    if not expected:
...
def test_remove_logs_for_pairs_already_in_blacklist(mocker, markets, static_pl_conf, caplog):
    ...
    for _ in range(3):
        new_whitelist = panther.pairlists.verify_blacklist(
...
def test_refresh_pairlist_dynamic(mocker, shitcoinmarkets, tickers, whitelist_conf):
    ...
    panther = get_patched_pantherbot(mocker, whitelist_conf)
    # Remock markets with shitcoinmarkets since get_patched_pantherbot uses the markets fixture
    mocker.patch.multiple(
        'panther.exchange.Exchange',
    ...
    panther.pairlists.refresh_pairlist()
    assert whitelist == panther.pairlists.whitelist

    whitelist_conf['pairlists'] = [{'method': 'VolumePairList'}]
    with pytest.raises(OperationalException,
                       match=r'`number_assets` not specified. Please check your configuration '
                             r'for "pairlist.config.number_assets"'):
        PairListManager(panther.exchange, whitelist_conf)
...
def test_refresh_pairlist_dynamic_2(mocker, shitcoinmarkets, tickers, whitelist_conf_2):
    ...
    mocker.patch.multiple(
        'panther.plugins.pairlistmanager.PairListManager',
        _get_cached_tickers=MagicMock(return_value=tickers_dict),
    )
    panther = get_patched_pantherbot(mocker, whitelist_conf_2)
    # Remock markets with shitcoinmarkets since get_patched_pantherbot uses the markets fixture
    mocker.patch.multiple(
        'panther.exchange.Exchange',
        markets=PropertyMock(return_value=shitcoinmarkets),
...
def test_VolumePairList_refresh_empty(mocker, markets_empty, whitelist_conf):
    ...
    panther = get_patched_pantherbot(mocker, whitelist_conf)
    mocker.patch('panther.exchange.Exchange.markets', PropertyMock(return_value=markets_empty))
...
def test_VolumePairList_whitelist_gen(mocker, whitelist_conf, shitcoinmarkets, tickers,
    ...
    mocker.patch.multiple('panther.persistence.Trade',
    ...
    if whitelist_result == 'filter_at_the_beginning':
        ...
        panther.pairlists.refresh_pairlist()
        whitelist = panther.pairlists.whitelist
...
def test_VolumePairList_range(mocker, whitelist_conf, shitcoinmarkets, tickers, ohlcv_history,
    ...
    mocker.patch('panther.exchange.ftx.Ftx.market_is_tradable', return_value=True)
...
def test_gen_pair_whitelist_not_supported(mocker, default_conf, tickers) -> None:
    ...
    with pytest.raises(OperationalException,
        ...
        get_patched_pantherbot(mocker, default_conf)
...
def test_pairlist_class(mocker, whitelist_conf, markets, pairlist):
    ...
    assert panther.pairlists.name_list == [pairlist]
    assert pairlist in str(panther.pairlists.short_desc())
    assert isinstance(panther.pairlists.whitelist, list)
    assert isinstance(panther.pairlists.blacklist, list)
...
@pytest.mark.parametrize("whitelist,log_message", [
    ...
    (['ETH/BTC', 'TKN/BTC', 'XLTCUSDT'], "is not tradable with Panther"),
])
def test__whitelist_for_active_markets(mocker, whitelist_conf, markets, pairlist, whitelist, caplog,
    ...
    # Assign starting whitelist
    pairlist_handler = panther.pairlists._pairlist_handlers[0]
...
def test__whitelist_for_active_markets_empty(mocker, whitelist_conf, pairlist, tickers):
    ...
    mocker.patch('panther.exchange.Exchange.exchange_has', return_value=True)
...
def test_volumepairlist_invalid_sortvalue(mocker, whitelist_conf):
    ...
    with pytest.raises(OperationalException,
        ...
        get_patched_pantherbot(mocker, whitelist_conf)
...
def test_volumepairlist_caching(mocker, markets, whitelist_conf, tickers):
    ...
    panther = get_patched_pantherbot(mocker, whitelist_conf)
    assert len(panther.pairlists._pairlist_handlers[0]._pair_cache) == 0
    assert tickers.call_count == 0
    panther.pairlists.refresh_pairlist()
    assert tickers.call_count == 1

    assert len(panther.pairlists._pairlist_handlers[0]._pair_cache) == 1
    panther.pairlists.refresh_pairlist()
    assert tickers.call_count == 1
...
def test_agefilter_caching(mocker, markets, whitelist_conf_agefilter, tickers, ohlcv_history):
    with time_machine.travel("2021-09-01 05:00:00 +00:00") as t:
        ...
        panther = get_patched_pantherbot(mocker, whitelist_conf_agefilter)
        assert panther.exchange.refresh_latest_ohlcv.call_count == 0
        panther.pairlists.refresh_pairlist()
        assert len(panther.pairlists.whitelist) == 3
        assert panther.exchange.refresh_latest_ohlcv.call_count > 0

        panther.pairlists.refresh_pairlist()
        assert len(panther.pairlists.whitelist) == 3
        # Call to XRP/BTC cached
        assert panther.exchange.refresh_latest_ohlcv.call_count == 2
        ...
        mocker.patch('panther.exchange.Exchange.refresh_latest_ohlcv', return_value=ohlcv_data)
        panther.pairlists.refresh_pairlist()
        assert len(panther.pairlists.whitelist) == 3
        assert panther.exchange.refresh_latest_ohlcv.call_count == 1
        ...
        mocker.patch('panther.exchange.Exchange.refresh_latest_ohlcv', return_value=ohlcv_data)
        panther.pairlists.refresh_pairlist()
        assert len(panther.pairlists.whitelist) == 4
...
def test_rangestabilityfilter_caching(mocker, markets, default_conf, tickers, ohlcv_history,
    ...
    assert panther.exchange.refresh_latest_ohlcv.call_count == 0
    panther.pairlists.refresh_pairlist()
    assert len(panther.pairlists.whitelist) == expected_length
    assert panther.exchange.refresh_latest_ohlcv.call_count > 0

    previous_call_count = panther.exchange.refresh_latest_ohlcv.call_count
    panther.pairlists.refresh_pairlist()
    assert len(panther.pairlists.whitelist) == expected_length
    # Should not have increased since first call.
    assert panther.exchange.refresh_latest_ohlcv.call_count == previous_call_count
...
def test_spreadfilter_invalid_data(mocker, default_conf, markets, tickers, caplog):
    ...
    mocker.patch.multiple('panther.exchange.Exchange', get_tickers=tickers)
...
def test_pricefilter_desc(mocker, whitelist_conf, markets, pairlistconfig,
    ...
    if desc_expected is not None:
        panther = get_patched_pantherbot(mocker, whitelist_conf)
        short_desc = str(panther.pairlists.short_desc())
...
def test_performance_filter(mocker, whitelist_conf, pairlists, pair_allowlist, overall_performance,
    ...
    panther = get_patched_pantherbot(mocker, allowlist_conf)
    mocker.patch.multiple('panther.exchange.Exchange',
                          get_tickers=tickers,
                          markets=PropertyMock(return_value=markets)
                          )
    ...
    panther.pairlists.refresh_pairlist()
    allowlist = panther.pairlists.whitelist
