...
from tests.conftest import get_patched_pantherbot, log_has_re
...
def test_protectionmanager(mocker, default_conf):
    ...
    for handler in panther.protections._protection_handlers:
...
def test_stoploss_guard(mocker, default_conf, fee, caplog):
    ...
    assert not panther.protections.global_stop()
    ...
    assert panther.protections.global_stop()
    ...
    panther.protections.global_stop(end_time)
...
def test_stoploss_guard_perpair(mocker, default_conf, fee, caplog, only_per_pair):
    ...
    message = r"Trading stopped due to .*"
    pair = 'XRP/BTC'
    assert not panther.protections.stop_per_pair(pair)
    assert not panther.protections.global_stop()
    assert not log_has_re(message, caplog)
    caplog.clear()
    ...
    # 3 Trades closed - but the 2nd has been closed too long ago.
    assert not panther.protections.stop_per_pair(pair)
    assert panther.protections.global_stop() != only_per_pair
    ...
    panther.protections.stop_per_pair(pair)
    assert panther.protections.global_stop() != only_per_pair
...
def test_CooldownPeriod(mocker, default_conf, fee, caplog):
    ...
    message = r"Trading stopped due to .*"
    assert not panther.protections.global_stop()
    assert not panther.protections.stop_per_pair('XRP/BTC')
    ...
    assert not panther.protections.global_stop()
    assert panther.protections.stop_per_pair('XRP/BTC')
    ...
    assert panther.protections.stop_per_pair('ETH/BTC')
...
def test_protection_manager_desc(mocker, default_conf, protectionconf,
    ...
    short_desc = str(panther.protections.short_desc())
