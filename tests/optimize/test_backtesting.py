...
from panther.commands.optimize_commands import setup_optimize_configuration, start_backtesting
from panther.configuration import TimeRange
from panther.data import history
from panther.data.btanalysis import BT_DATA_COLUMNS, evaluate_result_multi
from panther.data.converter import clean_ohlcv_dataframe
from panther.data.dataprovider import DataProvider
from panther.data.history import get_timerange
from panther.enums import RunMode, SellType
from panther.exceptions import DependencyException, OperationalException
from panther.misc import get_strategy_run_id
from panther.optimize.backtesting import Backtesting
from panther.persistence import LocalTrade
from panther.resolvers import StrategyResolver
...
def test_setup_bt_configuration_with_arguments(mocker, default_conf, caplog) -> None:
    patched_configuration_load_config_file(mocker, default_conf)
    mocker.patch(
        'panther.configuration.configuration.create_datadir',
        lambda c, x: x
    )
...
def test_start(mocker, fee, default_conf, caplog) -> None:
    ...
    mocker.patch('panther.exchange.Exchange.get_fee', fee)
    patch_exchange(mocker)
    mocker.patch('panther.optimize.backtesting.Backtesting.start', start_mock)
    patched_configuration_load_config_file(mocker, default_conf)
    ...
    assert log_has('Starting panther in Backtesting mode', caplog)
...
def test_backtesting_init(mocker, default_conf, order_types) -> None:
    ...
    patch_exchange(mocker)
    get_fee = mocker.patch('panther.exchange.Exchange.get_fee', MagicMock(return_value=0.5))
...
def test_backtesting_init_no_timeframe(mocker, default_conf, caplog) -> None:
    ...
    mocker.patch('panther.exchange.Exchange.get_fee', MagicMock(return_value=0.5))
...
def test_data_with_fee(default_conf, mocker, testdatadir) -> None:
    ...
    fee_mock = mocker.patch('panther.exchange.Exchange.get_fee', MagicMock(return_value=0.5))
    backtesting = Backtesting(default_conf)
    backtesting._set_strategy(backtesting.strategylist[0])
...
def test_backtesting_start(default_conf, mocker, testdatadir, caplog) -> None:
    ...
    mocker.patch('panther.data.history.get_timerange', get_timerange)
    patch_exchange(mocker)
    mocker.patch('panther.optimize.backtesting.Backtesting.backtest')
    mocker.patch('panther.optimize.backtesting.generate_backtest_stats')
    mocker.patch('panther.optimize.backtesting.show_backtest_results')
    sbs = mocker.patch('panther.optimize.backtesting.store_backtest_stats')
    mocker.patch('panther.plugins.pairlistmanager.PairListManager.whitelist',
...
def test_backtesting_start_no_data(default_conf, mocker, caplog, testdatadir) -> None:
    ...
    mocker.patch('panther.data.history.history_utils.load_pair_history',
...
def test_backtesting_pairlist_list(default_conf, mocker, caplog, testdatadir, tickers) -> None:
    ...
    mocker.patch('panther.plugins.pairlistmanager.PairListManager.refresh_pairlist')
...
def test_backtest__enter_trade(default_conf, fee, mocker) -> None:
    ...
    mocker.patch('panther.exchange.Exchange.get_fee', fee)
    mocker.patch("panther.exchange.Exchange.get_min_pair_stake_amount", return_value=0.00001)
    patch_exchange(mocker)
    ...
    mocker.patch("panther.exchange.Exchange.get_min_pair_stake_amount", return_value=600.0)

    trade = backtesting._enter_trade(pair, row=row)
    assert trade is None

    # Stake-amount throwing error
    mocker.patch("panther.wallets.Wallets.get_trade_stake_amount",
...
def test_backtest_start_multi_strat(default_conf, mocker, caplog, testdatadir):
    ...
    mocker.patch('panther.plugins.pairlistmanager.PairListManager.whitelist',
                 PropertyMock(return_value=['UNITTEST/BTC']))
    mocker.patch('panther.optimize.backtesting.Backtesting.backtest', backtestmock)
    text_table_mock = MagicMock()
    sell_reason_mock = MagicMock()
    strattable_mock = MagicMock()
    strat_summary = MagicMock()

    mocker.patch.multiple('panther.optimize.optimize_reports',
...
def test_backtest_start_multi_strat_caching(default_conf, mocker, caplog, testdatadir, run_id,
    ...
    mocker.patch('panther.plugins.pairlistmanager.PairListManager.whitelist',
                 PropertyMock(return_value=['UNITTEST/BTC']))
    mocker.patch('panther.optimize.backtesting.Backtesting.backtest', backtestmock)
    mocker.patch('panther.optimize.backtesting.show_backtest_results', MagicMock())
    ...
    mocker.patch.multiple('panther.data.btanalysis',
                          load_backtest_metadata=load_backtest_metadata,
                          load_backtest_stats=load_backtest_stats)
    mocker.patch('panther.optimize.backtesting.get_strategy_run_id', side_effect=['1', '2', '2'])
