...
from panther.__init__ import __version__
from panther.enums import RunMode, State
from panther.exceptions import DependencyException, ExchangeError, OperationalException
from panther.loggers import setup_logging, setup_logging_pre
from panther.persistence import PairLocks, Trade
from panther.rpc import RPC
from panther.rpc.api_server import ApiServer
from panther.rpc.api_server.api_auth import create_token, get_user_from_token
from panther.rpc.api_server.uvicorn_threaded import UvicornServer
from tests.conftest import (create_mock_trades, get_mock_coro, get_patched_pantherbot, log_has,
...
def botclient(default_conf, mocker):
    ...
    rpc = RPC(ftbot)
    mocker.patch('panther.rpc.api_server.ApiServer.start_api', MagicMock())
    try:
...
def test_api_ui_fallback(botclient, mocker):
    ...
    assert rc.status_code == 200
    assert '`panther install-ui`' in rc.text
    ...
    assert '`panther install-ui`' in rc.text or '<!DOCTYPE html>' in rc.text
...
def test_api_ui_version(botclient, mocker):
    ...
    mocker.patch('panther.commands.deploy_commands.read_ui_version', return_value='0.1.2')
...
def test_api_auth():
    with pytest.raises(ValueError):
        create_token({'identity': {'u': 'Panther'}}, 'secret1234', token_type="NotATokenType")

    token = create_token({'identity': {'u': 'Panther'}}, 'secret1234')
    ...
    token = create_token({'identity': {'u1': 'Panther'}}, 'secret1234')
    with pytest.raises(HTTPException):
...
def test_api__init__(default_conf, mocker):
    ...
    mocker.patch('panther.rpc.api_server.webserver.ApiServer.start_api', MagicMock())
    apiserver = ApiServer(default_conf)
    apiserver.add_rpc_handler(RPC(get_patched_pantherbot(mocker, default_conf)))
...
def test_api_UvicornServer(mocker):
    thread_mock = mocker.patch('panther.rpc.api_server.uvicorn_threaded.threading.Thread')
...
def test_api_UvicornServer_run(mocker):
    serve_mock = mocker.patch('panther.rpc.api_server.uvicorn_threaded.UvicornServer.serve',
                              get_mock_coro(None))
    s = UvicornServer(uvicorn.Config(MagicMock(), port=8080, host='127.0.0.1'))
...
def test_api_run(default_conf, mocker, caplog):
    ...
    mocker.patch('panther.rpc.api_server.webserver.UvicornServer', server_mock)
    ...
    caplog.clear()
    mocker.patch('panther.rpc.api_server.webserver.UvicornServer',
...
def test_api_show_config(botclient):
    ...
    assert rc.json()['bot_name'] == 'panther'
...
def test_api_status(botclient, mocker, ticker, fee, markets):
    ...
    mocker.patch('panther.exchange.Exchange.get_rate',
...
def test_api_backtesting(botclient, mocker, fee, caplog, tmpdir):
    ...
    mocker.patch('panther.optimize.backtesting.Backtesting.backtest_one_strategy',
