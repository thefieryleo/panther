...
from panther.constants import CANCEL_REASON
from panther.edge import PairInfo
...
from panther.loggers import setup_logging
from panther.persistence import PairLocks, Trade
from panther.rpc import RPC
from panther.rpc.rpc import RPCException
from panther.rpc.telegram import Telegram, authorized_only
from tests.conftest import (create_mock_trades, get_patched_pantherbot, log_has, log_has_re,
...
def get_telegram_testobject(mocker, default_conf, mock=True, ftbot=None):
    ...
    if mock:
        mocker.patch.multiple(
            'panther.rpc.telegram.Telegram',
            _init=MagicMock(),
            _send_msg=msg_mock
        )
    if not ftbot:
        ftbot = get_patched_pantherbot(mocker, default_conf)
    rpc = RPC(ftbot)
...
def test_telegram__init__(default_conf, mocker) -> None:
    mocker.patch('panther.rpc.telegram.Updater', MagicMock())
    mocker.patch('panther.rpc.telegram.Telegram._init', MagicMock())
...
def test_telegram_init(default_conf, mocker, caplog) -> None:
    ...
    mocker.patch('panther.rpc.telegram.Updater', MagicMock(return_value=start_polling))
...
def test_cleanup(default_conf, mocker, ) -> None:
    ...
    mocker.patch('panther.rpc.telegram.Updater', updater_mock)
...
def test_telegram_status(default_conf, update, mocker) -> None:
    ...
    mocker.patch('panther.rpc.telegram.Telegram._status_table', status_table)
...
def test_status_handle(default_conf, update, ticker, fee, mocker) -> None:
    ...
    telegram, pantherbot, msg_mock = get_telegram_testobject(mocker, default_conf)

    patch_get_signal(pantherbot)

    pantherbot.state = State.STOPPED
    ...
    pantherbot.state = State.RUNNING
    telegram._status(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert 'no active trade' in msg_mock.call_args_list[0][0][0]
    msg_mock.reset_mock()
...
def test_daily_handle(default_conf, update, ticker, limit_buy_order, fee,
    ...
    mocker.patch(
        'panther.rpc.rpc.CryptoToFiatConverter._find_price',
    ...
    # Reset msg_mock
    msg_mock.reset_mock()
    pantherbot.config['max_open_trades'] = 2
    # Add two other trades
    n = pantherbot.enter_positions()
    assert n == 2
...
def test_profit_handle(default_conf, update, ticker, ticker_sell_up, fee,
    ...
    mocker.patch('panther.rpc.rpc.CryptoToFiatConverter._find_price', return_value=15000.0)
    mocker.patch.multiple(
        'panther.exchange.Exchange',
        fetch_ticker=ticker,
        get_fee=fee,
    ...
    mocker.patch('panther.wallets.Wallets.get_starting_balance', return_value=0.01)
    assert ('∙ `-0.00000500 BTC (-0.50%) (-0.0 \N{GREEK CAPITAL LETTER SIGMA}%)`'
            in msg_mock.call_args_list[-1][0][0])
    msg_mock.reset_mock()

    # Update the ticker with a market going up
    mocker.patch('panther.exchange.Exchange.fetch_ticker', ticker_sell_up)
    trade.update(limit_sell_order)
...
def test_telegram_balance_handle(default_conf, update, mocker, rpc_balance, tickers) -> None:
    ...
    mocker.patch('panther.exchange.Exchange.get_balances', return_value=rpc_balance)
    mocker.patch('panther.exchange.Exchange.get_tickers', tickers)
    mocker.patch('panther.exchange.Exchange.get_valid_pair_combination',
...
def test_balance_handle_empty_response(default_conf, update, mocker) -> None:
    default_conf['dry_run'] = False
    mocker.patch('panther.exchange.Exchange.get_balances', return_value={})

    telegram, pantherbot, msg_mock = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(pantherbot)

    pantherbot.config['dry_run'] = False
    telegram._balance(update=update, context=MagicMock())
    result = msg_mock.call_args_list[0][0][0]
    assert msg_mock.call_count == 1
...
def test_balance_handle_too_large_response(default_conf, update, mocker) -> None:
    ...
    mocker.patch('panther.rpc.rpc.RPC._rpc_balance', return_value={
        'currencies': balances,
        'total': 100.0,
        'symbol': 100.0,
        'value': 1000.0,
...
def test_start_handle(default_conf, update, mocker) -> None:
    ...
    pantherbot.state = State.STOPPED
    assert pantherbot.state == State.STOPPED
    telegram._start(update=update, context=MagicMock())
    assert pantherbot.state == State.RUNNING
    assert msg_mock.call_count == 1
...
def test_stopbuy_handle(default_conf, update, mocker) -> None:
    ...
    assert pantherbot.config['max_open_trades'] != 0
...
def test_reload_config_handle(default_conf, update, mocker) -> None:
    ...
    assert pantherbot.state == State.RELOAD_CONFIG
    assert msg_mock.call_count == 1
...
def test_telegram_forcesell_handle(default_conf, update, ticker, fee,
    ...
    mocker.patch('panther.rpc.rpc.CryptoToFiatConverter._find_price', return_value=15000.0)
    msg_mock = mocker.patch('panther.rpc.telegram.Telegram.send_msg', MagicMock())
    mocker.patch('panther.rpc.telegram.Telegram._init', MagicMock())
    patch_exchange(mocker)
    ...
    pantherbot = PantherBot(default_conf)
    rpc = RPC(pantherbot)
    telegram = Telegram(rpc, default_conf)
    patch_get_signal(pantherbot)
...
def test_telegram_forcesell_down_handle(default_conf, update, ticker, fee,
    ...
    mocker.patch('panther.rpc.fiat_convert.CryptoToFiatConverter._find_price',
                 return_value=15000.0)
    msg_mock = mocker.patch('panther.rpc.telegram.Telegram.send_msg', MagicMock())
    mocker.patch('panther.rpc.telegram.Telegram._init', MagicMock())
    patch_exchange(mocker)
...
def test_forcebuy_handle(default_conf, update, mocker) -> None:
    ...
    mocker.patch('panther.rpc.RPC._rpc_forcebuy', fbuy_mock)

    telegram, pantherbot, _ = get_telegram_testobject(mocker, default_conf)
    patch_get_signal(pantherbot)
...
def test_telegram_buy_tag_performance_handle(default_conf, update, ticker, fee,
    ...
    msg_mock.reset_mock()
    mocker.patch('panther.rpc.rpc.RPC._rpc_buy_tag_performance',
...
def test_telegram_sell_reason_performance_handle(default_conf, update, ticker, fee,
    ...
    msg_mock.reset_mock()
    mocker.patch('panther.rpc.rpc.RPC._rpc_sell_reason_performance',
                 side_effect=RPCException('Error'))
...
def test_telegram_mix_tag_performance_handle(default_conf, update, ticker, fee,
    ...
    msg_mock.reset_mock()
    mocker.patch('panther.rpc.rpc.RPC._rpc_mix_tag_performance',
                 side_effect=RPCException('Error'))
...
def test_blacklist_static(default_conf, update, mocker) -> None:
    ...
    assert ("Blacklist contains 3 pairs\n`DOGE/BTC, HOT/BTC, ETH/BTC`"
            in msg_mock.call_args_list[0][0][0])
    assert pantherbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC"]
    ...
    assert ("Blacklist contains 4 pairs\n`DOGE/BTC, HOT/BTC, ETH/BTC, XRP/.*`"
            in msg_mock.call_args_list[0][0][0])
    assert pantherbot.pairlists.blacklist == ["DOGE/BTC", "HOT/BTC", "ETH/BTC", "XRP/.*"]
...
def test_telegram_logs(default_conf, update, mocker) -> None:
    ...
    assert msg_mock.call_count == 1
    assert "panther\\.rpc\\.telegram" in msg_mock.call_args_list[0][0][0]
    ...
    mocker.patch('panther.rpc.telegram.MAX_TELEGRAM_MESSAGE_LENGTH', 200)
    context = MagicMock()
    context.args = []
    telegram._logs(update=update, context=context)
...
def test_edge_enabled(edge_conf, update, mocker) -> None:
    mocker.patch('panther.edge.Edge._cached_pairs', mocker.PropertyMock(
        return_value={
...
def test_version_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    pantherbot.strategy.version = lambda: '1.1.1'
...
def test_show_config_handle(default_conf, update, mocker) -> None:
    ...
    msg_mock.reset_mock()
    pantherbot.config['trailing_stop'] = True
    telegram._show_config(update=update, context=MagicMock())
    assert msg_mock.call_count == 1
    assert '*Mode:* `{}`'.format('Dry-run') in msg_mock.call_args_list[0][0][0]
    assert '*Exchange:* `binance`' in msg_mock.call_args_list[0][0][0]
...
def test_send_msg_buy_notification(default_conf, mocker, caplog) -> None:
    ...
    pantherbot.config['telegram']['notification_settings'] = {'buy': 'off'}
    caplog.clear()
    msg_mock.reset_mock()
    telegram.send_msg(msg)
    ...
    pantherbot.config['telegram']['notification_settings'] = {'buy': 'silent'}
    caplog.clear()
    msg_mock.reset_mock()
...
def test__send_msg_keyboard(default_conf, mocker, caplog) -> None:
    ...
    pantherbot = get_patched_pantherbot(mocker, default_conf)
    rpc = RPC(pantherbot)
    ...
    pantherbot.config['telegram']['enabled'] = True
    telegram = init_telegram(pantherbot)
    telegram._send_msg('test')
    ...
    pantherbot.config['telegram']['enabled'] = True
    pantherbot.config['telegram']['keyboard'] = invalid_keys_list
    ...
    pantherbot.config['telegram']['enabled'] = True
    pantherbot.config['telegram']['keyboard'] = custom_keys_list
    telegram = init_telegram(pantherbot)
    telegram._send_msg('test')
    used_keyboard = bot.send_message.call_args[1]['reply_markup']
