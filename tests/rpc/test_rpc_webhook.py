...
from panther.enums import RPCMessageType, SellType
from panther.rpc import RPC
from panther.rpc.webhook import Webhook
...
def get_webhook_dict() -> dict:
    return {
        "enabled": True,
        "url": "https://maker.ifttt.com/trigger/panther_test/with/key/c764udvJ5jfSlswVRukZZ2/",
        "webhookbuy": {
            "value1": "Buying {pair}",
...
def test__init__(mocker, default_conf):
    ...
    webhook = Webhook(RPC(get_patched_pantherbot(mocker, default_conf)), default_conf)
...
def test_send_msg_webhook(default_conf, mocker):
    ...
    msg_mock = MagicMock()
    mocker.patch("panther.rpc.webhook.Webhook._send_msg", msg_mock)
    webhook = Webhook(RPC(get_patched_pantherbot(mocker, default_conf)), default_conf)
    # Test buy
    msg_mock = MagicMock()
    mocker.patch("panther.rpc.webhook.Webhook._send_msg", msg_mock)
...
def test__send_msg(default_conf, mocker, caplog):
    ...
    msg = {'value1': 'DEADBEEF',
           ...
           'value3': 'PANTHER'}
    post = MagicMock()
    mocker.patch("panther.rpc.webhook.post", post)
