...
from panther.enums import State
from panther.exceptions import ExchangeError, InvalidOrderException, TemporaryError
from panther.persistence import Trade
from panther.persistence.pairlock_middleware import PairLocks
from panther.rpc import RPC, RPCException
from panther.rpc.fiat_convert import CryptoToFiatConverter
from tests.conftest import create_mock_trades, get_patched_pantherbot, patch_get_signal
...
def test_rpc_trade_status(default_conf, ticker, fee, mocker) -> None:
    ...
    pantherbot.exit_positions(trades)
...
def test_rpc_delete_trade(mocker, default_conf, fee, markets, caplog):
    ...
    pantherbot = get_patched_pantherbot(mocker, default_conf)
    pantherbot.strategy.order_types['stoploss_on_exchange'] = True
    create_mock_trades(fee)
    rpc = RPC(pantherbot)
    ...
    stoploss_mock = mocker.patch('panther.exchange.Exchange.cancel_stoploss_order',
    ...
    cancel_mock = mocker.patch('panther.exchange.Exchange.cancel_order',
                               side_effect=InvalidOrderException)
...
def test_rpc_forcesell(default_conf, ticker, fee, mocker) -> None:
    ...
    mocker.patch('panther.wallets.Wallets.get_free', return_value=1000)
    ...
    mocker.patch(
        'panther.exchange.Exchange._is_dry_limit_order_filled', MagicMock(return_value=False))
    pantherbot.enter_positions()
    ...
    mocker.patch(
        'panther.exchange.Exchange.fetch_order',
        side_effect=[{
            'id': '1234',
            'status': 'open',
            'type': 'limit',
    ...
    pantherbot.config['max_open_trades'] = 3
    pantherbot.enter_positions()
...
def test_rpcforcebuy(mocker, default_conf, ticker, fee, limit_buy_order_open) -> None:
    ...
    pair = 'XRP/BTC'
    pantherbot = get_patched_pantherbot(mocker, default_conf)
    pantherbot.config['stake_amount'] = 0
    patch_get_signal(pantherbot)
    rpc = RPC(pantherbot)
    pair = 'TKN/BTC'
    trade = rpc._rpc_forcebuy(pair, None)
...
def test_rpc_edge_enabled(mocker, edge_conf) -> None:
    ...
    pantherbot = get_patched_pantherbot(mocker, edge_conf)
