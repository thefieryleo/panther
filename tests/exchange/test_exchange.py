...
from panther.exceptions import (DDosProtection, DependencyException, InvalidOrderException,
                                  OperationalException, PricingError, TemporaryError)
from panther.exchange import Binance, Bittrex, Exchange, Kraken
from panther.exchange.common import (API_FETCH_ORDER_RETRY_COUNT, API_RETRY_COUNT,
                                       calculate_backoff, remove_credentials)
from panther.exchange.exchange import (market_is_active, timeframe_to_minutes, timeframe_to_msecs,
...
def ccxt_exceptionhandlers(mocker, default_conf, api_mock, exchange_name,
    ...
    with patch('panther.exchange.common.time.sleep'):
...
async def async_ccxt_exception(mocker, default_conf, api_mock, fun, mock_ccxt_fun,
    ...
    with patch('panther.exchange.common.asyncio.sleep', get_mock_coro(None)):
        with pytest.raises(DDosProtection):
...
def test_init_ccxt_kwargs(default_conf, mocker, caplog):
    mocker.patch('panther.exchange.Exchange._load_markets', MagicMock(return_value={}))
...
def test_exchange_resolver(default_conf, mocker, caplog):
    mocker.patch('panther.exchange.Exchange._init_ccxt', MagicMock(return_value=MagicMock()))
    mocker.patch('panther.exchange.Exchange._load_async_markets')
...
def test_amount_to_precision(default_conf, mocker, amount, precision_mode, precision, expected):
    ...
    # TICK_SIZE = 4
    mocker.patch('panther.exchange.Exchange.precisionMode',
                 PropertyMock(return_value=precision_mode))
    mocker.patch('panther.exchange.Exchange.markets', markets)
...
def test_get_min_pair_stake_amount(mocker, default_conf) -> None:
    ...
    # no pair found
    mocker.patch(
        'panther.exchange.Exchange.markets',
...
def test__load_async_markets(default_conf, mocker, caplog):
    ...
    mocker.patch('panther.exchange.Exchange.validate_pairs')
    mocker.patch('panther.exchange.Exchange.validate_timeframes')
    mocker.patch('panther.exchange.Exchange._load_markets')
    mocker.patch('panther.exchange.Exchange.validate_stakecurrency')
...
def test__load_markets(default_conf, mocker, caplog):
    ...
    mocker.patch('panther.exchange.Exchange._init_ccxt', MagicMock(return_value=api_mock))
    mocker.patch('panther.exchange.Exchange.validate_pairs')
    mocker.patch('panther.exchange.Exchange.validate_timeframes')
    mocker.patch('panther.exchange.Exchange._load_async_markets')
    mocker.patch('panther.exchange.Exchange.validate_stakecurrency')
...
def test_validate_pairs_exception(default_conf, mocker, caplog):
    caplog.set_level(logging.INFO)
    api_mock = MagicMock()
    mocker.patch('panther.exchange.Exchange.name', PropertyMock(return_value='Binance'))

    type(api_mock).markets = PropertyMock(return_value={})
    mocker.patch('panther.exchange.Exchange._init_ccxt', api_mock)
    mocker.patch('panther.exchange.Exchange.validate_timeframes')
    mocker.patch('panther.exchange.Exchange.validate_stakecurrency')
    mocker.patch('panther.exchange.Exchange._load_async_markets')
    ...
    mocker.patch('panther.exchange.Exchange.markets', PropertyMock(return_value={}))
    Exchange(default_conf)
...
def test_validate_timeframes_failed(default_conf, mocker):
    ...
    mocker.patch('panther.exchange.Exchange._init_ccxt', MagicMock(return_value=api_mock))
    mocker.patch('panther.exchange.Exchange._load_markets', MagicMock(return_value={}))
    mocker.patch('panther.exchange.Exchange.validate_pairs', MagicMock())
    with pytest.raises(OperationalException,
    ...
    with pytest.raises(OperationalException,
                       match=r"Timeframes < 1m are currently not supported by Panther."):
        Exchange(default_conf)
...
def test_create_order(default_conf, mocker, side, ordertype, rate, marketprice, exchange_name):
    ...
    mocker.patch('panther.exchange.Exchange.amount_to_precision', lambda s, x, y: y)
    mocker.patch('panther.exchange.Exchange.price_to_precision', lambda s, x, y: y)
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
...
async def test___async_get_candle_history_sort(default_conf, mocker, exchange_name):
    ...
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    sort_mock = mocker.patch('panther.exchange.exchange.sorted', MagicMock(side_effect=sort_data))
    ...
    exchange._api_async.fetch_ohlcv = get_mock_coro(ohlcv)
    # Reset sort mock
    sort_mock = mocker.patch('panther.exchange.sorted', MagicMock(side_effect=sort_data))
    # Test the OHLCV data sort
    res = await exchange._async_get_candle_history('ETH/BTC', default_conf['timeframe'])
    assert res[0] == 'ETH/BTC'
...
def test_get_historic_trades_notsupported(default_conf, mocker, caplog, exchange_name,
                                          trades_history):
    mocker.patch('panther.exchange.Exchange.exchange_has', return_value=False)
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
...
def test_cancel_order_dry_run(default_conf, mocker, exchange_name):
    default_conf['dry_run'] = True
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)
    mocker.patch('panther.exchange.Exchange._is_dry_limit_order_filled', return_value=True)
...
def test_cancel_stoploss_order_with_result(default_conf, mocker, exchange_name):
    default_conf['dry_run'] = False
    mocker.patch('panther.exchange.Exchange.fetch_stoploss_order', return_value={'for': 123})
    mocker.patch('panther.exchange.Ftx.fetch_stoploss_order', return_value={'for': 123})
    exchange = get_patched_exchange(mocker, default_conf, id=exchange_name)

    mocker.patch('panther.exchange.Exchange.cancel_stoploss_order',
                 return_value={'fee': {}, 'status': 'canceled', 'amount': 1234})
    mocker.patch('panther.exchange.Ftx.cancel_stoploss_order',
                 return_value={'fee': {}, 'status': 'canceled', 'amount': 1234})
    ...
    mocker.patch('panther.exchange.Exchange.fetch_stoploss_order',
                 side_effect=InvalidOrderException(""))
    mocker.patch('panther.exchange.Ftx.fetch_stoploss_order',
                 side_effect=InvalidOrderException(""))
...
def test_fetch_order(default_conf, mocker, exchange_name, caplog):
    ...
    exchange = get_patched_exchange(mocker, default_conf, api_mock, id=exchange_name)
    with patch('panther.exchange.common.time.sleep') as tm:
        with pytest.raises(InvalidOrderException):
            exchange.fetch_order(order_id='_', pair='TKN/BTC')
...
def test_get_trades_for_order(default_conf, mocker, exchange_name):
    ...
    mocker.patch('panther.exchange.Exchange.exchange_has', MagicMock(return_value=False))
...
def test_get_markets_error(default_conf, mocker):
    ex = get_patched_exchange(mocker, default_conf)
    mocker.patch('panther.exchange.Exchange.markets', PropertyMock(return_value=None))
...
def test_extract_cost_curr_rate(mocker, default_conf, order, expected) -> None:
    mocker.patch('panther.exchange.Exchange.calculate_fee_rate', MagicMock(return_value=0.01))
    ex = get_patched_exchange(mocker, default_conf)
...
def test_calculate_fee_rate(mocker, default_conf, order, expected, unknown_fee_rate) -> None:
    mocker.patch('panther.exchange.Exchange.fetch_ticker', return_value={'last': 0.081})
    if unknown_fee_rate:
