...
from panther.persistence import LocalTrade, Trade, init_db
...
def patch_exchange(mocker, api_mock=None, id='binance', mock_markets=True) -> None:
    mocker.patch('panther.exchange.Exchange._load_async_markets', MagicMock(return_value={}))
    mocker.patch('panther.exchange.Exchange.validate_pairs', MagicMock())
    mocker.patch('panther.exchange.Exchange.validate_timeframes', MagicMock())
    mocker.patch('panther.exchange.Exchange.validate_ordertypes', MagicMock())
    mocker.patch('panther.exchange.Exchange.validate_stakecurrency', MagicMock())
    mocker.patch('panther.exchange.Exchange.id', PropertyMock(return_value=id))
    mocker.patch('panther.exchange.Exchange.name', PropertyMock(return_value=id.title()))
    mocker.patch('panther.exchange.Exchange.precisionMode', PropertyMock(return_value=2))
    if mock_markets:
        if isinstance(mock_markets, bool):
            mock_markets = get_markets()
        mocker.patch('panther.exchange.Exchange.markets',
                     PropertyMock(return_value=mock_markets))

    if api_mock:
        mocker.patch('panther.exchange.Exchange._init_ccxt', MagicMock(return_value=api_mock))
    else:
        mocker.patch('panther.exchange.Exchange._init_ccxt', MagicMock())
...
def patch_wallet(mocker, free=999.9) -> None:
    mocker.patch('panther.wallets.Wallets.get_free', MagicMock(
        return_value=free
    ))


def patch_whitelist(mocker, conf) -> None:
    mocker.patch('panther.pantherbot.PantherBot._refresh_active_whitelist',
...
def patch_edge(mocker) -> None:
    ...
    mocker.patch('panther.edge.Edge.calculate', MagicMock(return_value=True))


# Functions for recurrent object patching


def patch_pantherbot(mocker, config) -> None:
    """
    ...
    :return: None
    """
    mocker.patch('panther.pantherbot.RPCManager', MagicMock())
    init_db(config['db_url'])
    patch_exchange(mocker)
    mocker.patch('panther.pantherbot.RPCManager._init', MagicMock())
    mocker.patch('panther.pantherbot.RPCManager.send_msg', MagicMock())
    patch_whitelist(mocker, config)


def get_patched_pantherbot(mocker, config) -> PantherBot:
    """
    ...
    :return: PantherBot
    """
    patch_pantherbot(mocker, config)
    config['datadir'] = Path(config['datadir'])
    return PantherBot(config)
...
def patch_get_signal(panther: PantherBot, value=(True, False, None, None)) -> None:
    """
    :param mocker: mocker to patch IStrategy class
    :param value: which value IStrategy.get_signal() must return
    :return: None
    """
    panther.strategy.get_signal = lambda e, s, x: value
    panther.exchange.refresh_latest_ohlcv = lambda p: None
...
def patch_coingekko(mocker) -> None:
    ...
    mocker.patch.multiple(
        'panther.rpc.fiat_convert.CoinGeckoAPI',
