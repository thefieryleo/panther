...
from panther.strategy.hyper import (BaseParameter, BooleanParameter, CategoricalParameter,
...
def test_advise_all_indicators_copy(mocker, default_conf, testdatadir) -> None:
    default_conf.update({'strategy': 'StrategyTestV2'})
    strategy = StrategyResolver.load_strategy(default_conf)
    aimock = mocker.patch('panther.strategy.interface.IStrategy.advise_indicators')
    timerange = TimeRange.parse_timerange('1510694220-1510700340')
    data = load_data(testdatadir, '1m', ['UNITTEST/BTC'], timerange=timerange,
                     fill_up_missing=True)
...
def test_analyze_ticker_default(ohlcv_history, mocker, caplog) -> None:
    ...
    mocker.patch.multiple(
        'panther.strategy.interface.IStrategy',
...
def test_auto_hyperopt_interface_loadparams(default_conf, mocker, caplog):
    ...
    mocker.patch('panther.strategy.hyper.json_load', return_value=expected_result)
    PairLocks.timeframe = default_conf['timeframe']
    strategy = StrategyResolver.load_strategy(default_conf)
    ...
    mocker.patch('panther.strategy.hyper.json_load', MagicMock(side_effect=ValueError()))
