...
from panther.strategy import (merge_informative_pair, stoploss_from_absolute, stoploss_from_open,
...
def generate_test_data(timeframe: str, size: int, start: str = '2020-07-05'):
    ...
    date = pd.date_range(start, periods=size, panther=f'{tf_mins}min', tz='UTC')
    df = pd.DataFrame({
        'date': date,
        'open': base,
...
def test_informative_decorator(mocker, default_conf):
    ...
    mocker.patch('panther.data.dataprovider.DataProvider.historic_ohlcv',
