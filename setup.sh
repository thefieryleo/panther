...
function updateenv() {
    ...
    if [ $? -ne 0 ]; then
        echo "Failed installing Panther"
        exit 1
    fi
...
function config() {
    echo_block "Please use 'panther new-config -c config.json' to generate a new configuration file."
}
...
function install() {
    ...
    echo "You can now use the bot by executing 'source .env/bin/activate; panther <subcommand>'."
    echo "You can see the list of available bot sub-commands by executing 'source .env/bin/activate; panther --help'."
    echo "You verify that panther is installed successfully by running 'source .env/bin/activate; panther --version'."
}
...
function help() {
    echo "usage:"
    echo "	-i,--install    Install panther from scratch"
