...
from panther.configuration.timerange import TimeRange
from panther.constants import CANCEL_REASON, DATETIME_PRINT_FORMAT
from panther.data.history import load_data
from panther.enums import SellType, State
from panther.exceptions import ExchangeError, PricingError
from panther.exchange import timeframe_to_minutes, timeframe_to_msecs
from panther.loggers import bufferHandler
from panther.misc import decimals_per_coin, shorten_date
from panther.persistence import PairLocks, Trade
from panther.persistence.models import PairLock
from panther.plugins.pairlist.pairlist_helpers import expand_pairlist
from panther.rpc.fiat_convert import CryptoToFiatConverter
...
class RPC:
    ...
    def __init__(self, panther) -> None:
        """
        Initializes all enabled rpc modules
        :param panther: Instance of a panther bot
        :return: None
        """
        self._panther = panther
        self._config: Dict[str, Any] = panther.config
    ...
    def _rpc_show_config(config, botstate: Union[State, str],
        ...
        val = {
            ...
            'bot_name': config.get('bot_name', 'panther'),
        ...
        if not trades:
            ...
            for trade in trades:
                ...
                if trade.open_order_id:
                    order = self._panther.exchange.fetch_order(trade.open_order_id, trade.pair)
                # calculate profit and send message to user
                if trade.is_open:
                    try:
                        current_rate = self._panther.exchange.get_rate(
                ...
                if self._fiat_converter:
                    current_profit_fiat = self._fiat_converter.convert_amount(
                        current_profit_abs,
                        self._panther.config['stake_currency'],
                        self._panther.config['fiat_display_currency']
                    )
                ...
                trade_dict.update(dict(
                    base_currency=self._panther.config['stake_currency'],
    ...
    def _rpc_trade_statistics(
        ...
        starting_balance = self._panther.wallets.get_starting_balance()
    ...
    def _rpc_balance(self, stake_currency: str, fiat_display_currency: str) -> Dict:
        ...
        total = 0.0
        try:
            tickers = self._panther.exchange.get_tickers(cached=True)
        except (ExchangeError):
            raise RPCException('Error getting current tickers.')

        self._panther.wallets.update(require_update=False)
        starting_capital = self._panther.wallets.get_starting_balance()
        ...
        for coin, balance in self._panther.wallets.get_all_balances().items():
            ...
            if coin == stake_currency:
                ...
                try:
                    pair = self._panther.exchange.get_valid_pair_combination(coin, stake_currency)
        ...
        if total == 0.0:
            if self._panther.config['dry_run']:
        ...
        return {
            ...
            'note': 'Simulated balances' if self._panther.config['dry_run'] else ''
        }

    def _rpc_start(self) -> Dict[str, str]:
        """ Handler for start """
        if self._panther.state == State.RUNNING:
            return {'status': 'already running'}

        self._panther.state = State.RUNNING
        return {'status': 'starting trader ...'}

    def _rpc_stop(self) -> Dict[str, str]:
        """ Handler for stop """
        if self._panther.state == State.RUNNING:
            self._panther.state = State.STOPPED
    ...
    def _rpc_reload_config(self) -> Dict[str, str]:
        ...
        self._panther.state = State.RELOAD_CONFIG
    ...
    def _rpc_stopbuy(self) -> Dict[str, str]:
        ...
        if self._panther.state == State.RUNNING:
            ...
            self._panther.config['max_open_trades'] = 0
    ...
    def _rpc_forcesell(self, trade_id: str, ordertype: Optional[str] = None) -> Dict[str, str]:
        ...
        def _exec_forcesell(trade: Trade) -> None:
            ...
            if trade.open_order_id:
                ...
                if order['side'] == 'buy':
                    fully_canceled = self._panther.handle_cancel_enter(
                        trade, order, CANCEL_REASON['FORCE_SELL'])

                if order['side'] == 'sell':
                    # Cancel order - so it is placed anew with a fresh price.
                    self._panther.handle_cancel_exit(trade, order, CANCEL_REASON['FORCE_SELL'])
            ...
            if not fully_canceled:
                ...
                order_type = ordertype or self._panther.strategy.order_types.get(
                    "forcesell", self._panther.strategy.order_types["sell"])

                self._panther.execute_trade_exit(
        ...
        with self._panther._exit_lock:
            if trade_id == 'all':
                # Execute sell for all open orders
                for trade in Trade.get_open_trades():
                    _exec_forcesell(trade)
                Trade.commit()
                self._panther.wallets.update()
    ...
    def _rpc_forcebuy(self, pair: str, price: Optional[float], order_type: Optional[str] = None,
        ...
        if not self._panther.config.get('forcebuy_enable', False):
        ...
        stake_currency = self._panther.config.get('stake_currency')
        if not self._panther.exchange.get_pair_quote_currency(pair) == stake_currency:
            raise RPCException(
        ...
        if trade:
            if not self._panther.strategy.position_adjustment_enable:
                raise RPCException(f'position for {pair} already open - id: {trade.id}')

        if not stake_amount:
            # gen stake amount
            stake_amount = self._panther.wallets.get_trade_stake_amount(pair)

        # execute buy
        if not order_type:
            order_type = self._panther.strategy.order_types.get(
                'forcebuy', self._panther.strategy.order_types['buy'])
        if self._panther.execute_entry(pair, stake_amount, price,
    ...
    def _rpc_delete(self, trade_id: int) -> Dict[str, Union[str, int]]:
        ...
        with self._panther._exit_lock:
            ...
            if trade.open_order_id:
                try:
                    self._panther.exchange.cancel_order(trade.open_order_id, trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass

            # cancel stoploss on exchange ...
            if (self._panther.strategy.order_types.get('stoploss_on_exchange')
                    and trade.stoploss_order_id):
                try:
                    self._panther.exchange.cancel_stoploss_order(trade.stoploss_order_id,
                                                                   trade.pair)
                    c_count += 1
                except (ExchangeError):
                    pass
    ...
    def _rpc_count(self) -> Dict[str, float]:
        ...
        return {
            'current': len(trades),
            'max': (int(self._panther.config['max_open_trades'])
                    if self._panther.config['max_open_trades'] != float('inf') else -1),
    ...
    def _rpc_whitelist(self) -> Dict:
        ...
        res = {'method': self._panther.pairlists.name_list,
               'length': len(self._panther.active_pair_whitelist),
               'whitelist': self._panther.active_pair_whitelist
               }
        return res
    ...
    def _rpc_blacklist_delete(self, delete: List[str]) -> Dict:
        ...
        errors = {}
        for pair in delete:
            if pair in self._panther.pairlists.blacklist:
                self._panther.pairlists.blacklist.remove(pair)
            else:
                errors[pair] = {
    ...
    def _rpc_blacklist(self, add: List[str] = None) -> Dict:
        ...
        if add:
            for pair in add:
                if pair not in self._panther.pairlists.blacklist:
                    try:
                        expand_pairlist([pair], self._panther.exchange.get_markets().keys())
                        self._panther.pairlists.blacklist.append(pair)
        ...
        res = {'method': self._panther.pairlists.name_list,
               'length': len(self._panther.pairlists.blacklist),
               'blacklist': self._panther.pairlists.blacklist,
               'blacklist_expanded': self._panther.pairlists.expanded_blacklist,
               'errors': errors,
               }
        return res
    ...
    def _rpc_get_logs(limit: Optional[int]) -> Dict[str, Any]:
        ...
        #       "panther.worker", "INFO", "Starting worker develop"]

        return {'log_count': len(records), 'logs': records}

    def _rpc_edge(self) -> List[Dict[str, Any]]:
        """ Returns information related to Edge """
        if not self._panther.edge:
            raise RPCException('Edge is not enabled.')
        return self._panther.edge.accepted_pairs()
    ...
    def _rpc_analysed_dataframe(self, pair: str, timeframe: str,
        ...
        _data, last_analyzed = self._panther.dataprovider.get_analyzed_dataframe(
            pair, timeframe)
        _data = _data.copy()
        if limit:
            _data = _data.iloc[-limit:]
        return self._convert_dataframe_to_dict(self._panther.config['strategy'],
    ...
    def _rpc_analysed_history_full(config, pair: str, timeframe: str,
        ...
        from panther.data.dataprovider import DataProvider
    ...
    def _rpc_plot_config(self) -> Dict[str, Any]:
        if (self._panther.strategy.plot_config and
                'subplots' not in self._panther.strategy.plot_config):
            self._panther.strategy.plot_config['subplots'] = {}
        return self._panther.strategy.plot_config
