...
from panther.rpc import RPC, RPCHandler
...
class RPCManager:
    ...
    def __init__(self, panther) -> None:
        ...
        self._rpc = RPC(panther)
        config = panther.config
        # Enable telegram
        if config.get('telegram', {}).get('enabled', False):
            logger.info('Enabling rpc.telegram ...')
            from panther.rpc.telegram import Telegram
    ...
    def send_msg(self, msg: Dict[str, Any]) -> None:
        ...
        if 'pair' in msg:
            msg.update({
                'base_currency': self._rpc._panther.exchange.get_pair_base_currency(msg['pair'])
                })
