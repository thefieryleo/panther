...
from panther.exceptions import OperationalException, TemporaryError
...
class Worker:
    ...
    def _init(self, reconfig: bool) -> None:
        ...
        self.panther = PantherBot(self._config)
    ...
    def _worker(self, old_state: Optional[State]) -> State:
        ...
        """
        state = self.panther.state

        # Log state transition
        if state != old_state:

            if old_state != State.RELOAD_CONFIG:
                self.panther.notify_status(f'{state.name.lower()}')

            logger.info(
                f"Changing state{f' from {old_state.name}' if old_state else ''} to: {state.name}")
            if state == State.RUNNING:
                self.panther.startup()

            if state == State.STOPPED:
                self.panther.check_for_open_trades()
        ...
        if self._heartbeat_interval:
            ...
            if (now - self._heartbeat_msg) > self._heartbeat_interval:
                version = __version__
                strategy_version = self.panther.strategy.version()
    ...
    def _process_stopped(self) -> None:
        self.panther.process_stopped()

    def _process_running(self) -> None:
        try:
            self.panther.process()
            ...
            self.panther.notify_status(f'OperationalException:\n```\n{tb}```{hint}')

            logger.exception('OperationalException. Stopping trader ...')
            self.panther.state = State.STOPPED

    def _reconfigure(self) -> None:
        """
        Cleans up current pantherbot instance, reloads the configuration and
        replaces it with the new instance
        """
        # Tell systemd that we initiated reconfiguration
        self._notify("RELOADING=1")

        # Clean up current panther modules
        self.panther.cleanup()

        # Load and validate config and create new instance of the bot
        self._init(True)

        self.panther.notify_status('config reloaded')
    ...
    def exit(self) -> None:
        ...
        if self.panther:
            self.panther.notify_status('process died')
            self.panther.cleanup()
