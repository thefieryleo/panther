...
def _extend_validator(validator_class):
    """
    Extended validator for the Panther configuration JSON Schema.
...
PantherValidator = _extend_validator(Draft4Validator)
...
def validate_config_schema(conf: Dict[str, Any]) -> Dict[str, Any]:
    ...
    try:
        PantherValidator(conf_schema).validate(conf)
        return conf
